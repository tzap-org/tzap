...
fn publish_dependencies_are_versioned() {
    ...
    assert!(manifest.contains(r#"tzap-core = { path = "../tzap-core", version = "0.1.2" }"#));
    assert!(manifest.contains(
        r#"tzap-plugin-signing = { path = "../tzap-plugin-signing", version = "0.1.1" }"#
    ));
    assert!(plugin_manifest.contains(r#"tzap-core = { path = "../tzap-core", version = "0.1.2" }"#));
}
...
fn package_readmes_render_without_workspace_paths() {
    ...
    assert!(signing_readme.contains("tzap-plugin-signing = \"0.1.1\""));
