...
pub fn verify_hmac(
    ...
    mac.update(session_id);
    mac.update(covered_bytes);
    mac.verify_slice(expected_hmac).map_err(|_| FormatError::HmacMismatch { structure: domain.structure_name() })
}
...
pub fn compute_integrity_tag(
    ...
    if aead_algo.is_encrypted() {
        return Ok(compute_hmac(domain, mac_key.ok_or(FormatError::KeyMaterialMismatch)?, archive_uuid, session_id, covered_bytes));
    }
...
pub fn verify_integrity_tag(
    ...
    if aead_algo.is_encrypted() {
        return verify_hmac(domain, mac_key.ok_or(FormatError::KeyMaterialMismatch)?, archive_uuid, session_id, covered_bytes, expected_tag);
    }
    ...
    if expected_tag == actual {
        ...
        Err(FormatError::IntegrityDigestMismatch { structure: domain.structure_name() })
    }
}
...
fn parse_raw_kdf_params(bytes: &[u8]) -> Result<(KdfParams, usize), FormatError> {
    ...
    if algo_tag != KdfAlgo::Raw as u16 {
        return Err(FormatError::KdfAlgoTagMismatch { expected: KdfAlgo::Raw as u16, actual: algo_tag });
    }
...
fn parse_none_kdf_params(bytes: &[u8]) -> Result<(KdfParams, usize), FormatError> {
    ...
    if algo_tag != KdfAlgo::None as u16 {
        return Err(FormatError::KdfAlgoTagMismatch { expected: KdfAlgo::None as u16, actual: algo_tag });
    }
...
fn parse_argon2id_kdf_params(bytes: &[u8]) -> Result<(KdfParams, usize), FormatError> {
    ...
    if algo_tag != KdfAlgo::Argon2id as u16 {
        return Err(FormatError::KdfAlgoTagMismatch { expected: KdfAlgo::Argon2id as u16, actual: algo_tag });
    }
    ...
    Ok((KdfParams::Argon2id { t_cost, m_cost_kib, parallelism, salt: bytes[ARGON2ID_FIXED_PARAMS_LEN..total_len].to_vec() }, total_len))
}
...
fn parse_recipient_wrap_kdf_params(bytes: &[u8]) -> Result<(KdfParams, usize), FormatError> {
    ...
    if algo_tag != KdfAlgo::RecipientWrap as u16 {
        return Err(FormatError::KdfAlgoTagMismatch { expected: KdfAlgo::RecipientWrap as u16, actual: algo_tag });
    }
...
fn validate_argon2id_bounds(t_cost: u32, m_cost_kib: u32, parallelism: u32, salt_length: u16) -> Result<(), FormatError> {
    ...
    if t_cost > READER_MAX_ARGON2ID_T_COST {
        return Err(FormatError::ReaderResourceLimitExceeded { field: "argon2id t_cost", cap: READER_MAX_ARGON2ID_T_COST as u64, actual: t_cost as u64 });
    }
    if parallelism == 0 {
        return Err(FormatError::InvalidKdfParams("argon2id parallelism must be non-zero"));
    }
    ...
    let memory_time_product = u64::from(t_cost).checked_mul(u64::from(m_cost_kib)).ok_or(FormatError::InvalidKdfParams("argon2id work product overflow"))?;
    ...
    let min_memory = parallelism.checked_mul(8).ok_or(FormatError::InvalidKdfParams("argon2id memory requirement overflow"))?;
...
fn validate_nonce_len(algo: AeadAlgo, nonce: &[u8]) -> Result<(), FormatError> {
    ...
    if nonce.len() != expected {
        return Err(FormatError::InvalidNonceLength { algo, expected, actual: nonce.len() });
    }
    Ok(())
}
...
mod tests {
    ...
    fn parses_none_kdf_params() {
        ...
        assert_eq!(
            ...
            FormatError::KdfAlgoTagMismatch { expected: KdfAlgo::None as u16, actual: KdfAlgo::Raw as u16 }
        );
    }
    ...
    fn parses_argon2id_kdf_params() {
        ...
        assert_eq!(params, KdfParams::Argon2id { t_cost: 1, m_cost_kib: 8, parallelism: 1, salt: b"12345678".to_vec() });
    }
    ...
    fn rejects_invalid_recipient_wrap_kdf_params_fields() {
        ...
        bytes.extend_from_slice(&0u16.to_le_bytes());
        bytes.extend_from_slice(&[0xaau8; 32]);
        assert_eq!(KdfParams::parse(KdfAlgo::RecipientWrap, &bytes).unwrap_err(), FormatError::InvalidKdfParams("recipient-wrap table version must be 1"));
    ...
    fn rejects_argon2id_params_above_reader_caps() {
        ...
        let err = MasterKey::derive_from_passphrase(
            &KdfParams::Argon2id { t_cost: 1, m_cost_kib: READER_MAX_ARGON2ID_M_COST_KIB + 1, parallelism: 1, salt: b"12345678".to_vec() },
            "passphrase",
        )
        .unwrap_err();
        assert_eq!(
    ...
    fn rejects_argon2id_salt_bounds_and_raw_kdf_truncation() {
        ...
        assert!(matches!(KdfParams::parse(KdfAlgo::Argon2id, &argon_bytes(8, b"12345678")).unwrap(), (KdfParams::Argon2id { .. }, 24)));
        assert!(matches!(KdfParams::parse(KdfAlgo::Argon2id, &argon_bytes(64, &[0x5a; 64])).unwrap(), (KdfParams::Argon2id { .. }, 80)));
        assert_eq!(
            KdfParams::parse(KdfAlgo::Argon2id, &argon_bytes(65, &[0x5a; 65])).unwrap_err(),
            FormatError::InvalidKdfParams("argon2id salt length must be 8..64 bytes")
        );
        assert_eq!(KdfParams::parse(KdfAlgo::Argon2id, &argon_bytes(64, &[0x5a; 63])).unwrap_err(), FormatError::TruncatedKdfParams);
    }
    ...
    fn argon2id_passphrase_edge_vectors_are_literal() {
        ...
        let cases = [
            ...
            ("leading bom", "\u{feff}pass", "d493645da269dce9b0ab6d39367d94c1896b0f4a2c3ca486c775d7275b8558da"),
        ];
    ...
    fn hkdf_passphrase_and_identity_vectors_are_literal() {
        let params = KdfParams::Argon2id { t_cost: 1, m_cost_kib: 8, parallelism: 1, salt: b"saltsalt".to_vec() };
        ...
        assert_eq!(hex::encode(subkeys.index_nonce_seed), "5d51a19bf7f6d77ce7945517ce95837a089f8d1cd20aea43cbcb8d745c0668ee");
    ...
    fn computes_and_verifies_unkeyed_integrity_domains() {
        let covered = b"covered bytes";
        let tag_v45 = compute_integrity_tag(HmacDomain::CryptoHeader, AeadAlgo::None, VOLUME_FORMAT_REV_45, None, &uuid(), &session(), covered).unwrap();

        verify_integrity_tag(HmacDomain::CryptoHeader, AeadAlgo::None, VOLUME_FORMAT_REV_45, None, &uuid(), &session(), covered, &tag_v45).unwrap();

        assert_eq!(
            verify_integrity_tag(HmacDomain::ManifestFooter, AeadAlgo::None, VOLUME_FORMAT_REV_45, None, &uuid(), &session(), covered, &tag_v45,).unwrap_err(),
            FormatError::IntegrityDigestMismatch { structure: "ManifestFooter" }
        );

        let manifest_tag_v45 =
            compute_integrity_tag(HmacDomain::ManifestFooter, AeadAlgo::None, VOLUME_FORMAT_REV_45, None, &uuid(), &session(), covered).unwrap();
        assert_eq!(
            verify_integrity_tag(HmacDomain::ManifestFooter, AeadAlgo::None, VOLUME_FORMAT_REV_45, None, &uuid(), &session(), covered, &manifest_tag_v45,)
                .unwrap(),
            ()
        );
        assert_ne!(
    ...
    fn rejects_old_nonce_info_without_domain_length() {
        ...
        let ciphertext = encrypt_padded_aead_object(
            AeadObjectContext { algo: AeadAlgo::AesGcmSiv256, key: &key, nonce_seed: &nonce_seed, domain, archive_uuid: &uuid, session_id: &session, counter },
            4096,
            b"index-root",
        )
        .unwrap();
        let mut legacy_nonce = vec![0u8; AeadAlgo::AesGcmSiv256.nonce_len()];
        Hkdf::<Sha256>::from_prk(&nonce_seed).unwrap().expand(&legacy_nonce_info(domain, &uuid, &session, counter), &mut legacy_nonce).unwrap();
        ...
        assert_eq!(aead_decrypt(AeadAlgo::AesGcmSiv256, &key, &legacy_nonce, &aad, &ciphertext,).unwrap_err(), FormatError::AeadFailure);
    }
    ...
    fn aead_rejects_wrong_nonce_length() {
        assert_eq!(
            ...
            FormatError::InvalidNonceLength { algo: AeadAlgo::AesGcmSiv256, expected: 12, actual: 11 }
        );
    }
    ...
    fn padded_aead_object_round_trips_with_derived_nonce_and_aad() {
        ...
        assert_eq!(decrypt_padded_aead_object(AeadObjectContext { domain: b"idxroot", ..context }, &ciphertext,).unwrap_err(), FormatError::AeadFailure);
    }
    ...
    fn rejects_index_root_aad_counter_mismatch() {
        ...
        assert_eq!(aead_decrypt(AeadAlgo::AesGcmSiv256, &key, &nonce, &mismatched_aad, &ciphertext,).unwrap_err(), FormatError::AeadFailure);
    }
}
