# tzap Archive Format Specification (v0.31)

| Field | Value |
|---|---|
| **Format version** | 1 |
| **Document version** | 0.31 / 2026-05-22.1 (critical consistency fixes) |
| **Status** | Draft for review, no implementation yet |
| **Owner** | Frank Zhu |
| **Maintainers** | Frank Zhu |
| **Last updated** | 2026-05-22 |
| **Supersedes** | v0.1, v0.2, v0.3, v0.4, v0.5, v0.6, v0.7, v0.8, v0.9, v0.10, v0.11, v0.12, v0.13, v0.14, v0.15, v0.16, v0.17, v0.18, v0.19, v0.20, v0.21, v0.22, v0.23, v0.24, v0.25, v0.26, v0.27, v0.28, v0.29, v0.30 |
| **Superseded by** | None |
| **Conflict rule** | This document supersedes earlier tzap format drafts. If it conflicts with v0.1-v0.30 text, this v0.31 draft wins unless a later dated spec explicitly supersedes it. |
| **File extension** | `.tzap` (single-volume) / `.tzap.NNN` (multi-volume) |

## Changelog from v0.30

This revision resolves critical consistency findings in v0.30:
an impossible sidecar ManifestFooter byte-copy rule for volume 0,
sequential extraction output semantics that conflicted with the
truncation/plaintext-release guarantee, and missing early rejection for
zero-data-block encrypted objects.

1. **Draft visibility is bumped.** `volume_format_rev = 31` for this
   draft. (§8, §17.1, §23, §28.1, §29)
2. **Sidecar ManifestFooter serialization is made implementable.** A
   sidecar ManifestFooter remains a sidecar instance with
   `volume_index = 0`, but it is allowed to be byte-identical to the
   volume-0 ManifestFooter when all fields and HMAC inputs match. The
   forbidden operation is copying a nonzero-volume footer and mutating
   `volume_index` after HMAC. (§12.3, §19.1, §28.1, §29)
3. **Sequential extraction output is explicitly provisional until
   terminal authentication succeeds.** Live non-seekable readers may
   stream per-envelope-authenticated bytes, but default filesystem
   extractors must stage or quarantine writes and must not commit clean
   output until ManifestFooter/VolumeTrailer authentication succeeds.
   (§1, §3, §17.3, §21, §22, §28.1, §29)
4. **Present encrypted objects must contain at least one data block.**
   Readers reject zero-data-block IndexRoot, IndexShard, dictionary,
   directory-hint, and payload-envelope objects before FEC/AEAD work;
   empty archives still contain a non-empty IndexRoot. (§11, §15.3,
   §15.9, §18, §28.1, §29)
...
a usable archive mode. Conformant v0.31 writers MUST set
`compression_algo = ZstdFramed`; Conformant v0.31 readers MUST reject
...
this draft, not usable archive modes. Conformant v0.31 writers MUST set
`fec_algo = ReedSolomonGF16`; Conformant v0.31 readers MUST reject any
...
- `pad_len = 0` is not valid in v0.31. The extra block in the exact-fit
...
stream), conforming v0.31 writers MUST use `stripe_width = 1`,
...
struct VolumeHeader {
    ...
    volume_format_rev:        u16,       // 31 for this draft
...
parameters.
Readers MUST reject `compression_algo = None` in v0.31; payload and
...
known extensions are single-valued in v0.31; writers MUST NOT emit the
...
| ~~`0x0004`~~ | ~~`[u8; 32]`~~ | **Forbidden in v0.31.** The tar-stream content hash is encrypted inside IndexRoot. Writers MUST NOT emit this extension; readers MUST reject it if present. |
| `0x0005` | UTF-8 | Locale tag for filenames |
| ~~`0x0006`~~ | ~~bytes~~ | **Forbidden in v0.31; moved to encrypted metadata.** A writer setting `has_dictionary = 1` declares that IndexRoot locates a dictionary-object extent (§15.2). Writers MUST NOT emit this extension; readers MUST reject it if present. |
...
struct BlockRecord {
    ...
    flags:         u8,               // bit 0: last data block of encrypted object
                                     // bits 1..7: reserved; MUST be zero in v0.31
    _reserved:     [u8; 2],
...
MUST reject a BlockRecord with any reserved flag bit set; in v0.31 this
...
Completed v0.31 writers MUST set `is_authoritative = 1` in every closed
...
ManifestFooter and VolumeTrailer in v0.31.
...
struct BootstrapSidecarHeader {
    ...
    flags:                       u32,       // bit 0: ManifestFooter present
                                             ...
                                             // bits 3..31: reserved; MUST be zero in v0.31
...
be zero in v0.31; readers MUST reject the sidecar before trusting any
...
ManifestFooter HMAC and `archive_uuid`/`session_id`, MUST reject a v0.31
...
No padding, extension bytes, or unclaimed gaps are permitted in a v0.31
...
non-critical by a future version; for v0.31, unknown flag bits are a hard
error.
...
payload envelope sequence has no gaps. Because v0.31 stores only
...
For every encrypted object in v0.31, `encrypted_size` is the total
...
`FileEntry.flags` is reserved in v0.31. Writers MUST set it to zero, and
...
- when the same `frame_index` or `envelope_index` appears in more than
  ...
  version, this is equivalent to byte-identical row encoding in v0.31.
...
1. Read VolumeHeader at offset 0; verify CRC.
   ...
   by the reader. A reader claiming only v0.31 conformance rejects
   `volume_format_rev != 31`. Reject if `VolumeHeader.stripe_width = 0`
...
3. Reject unknown `compression_algo`, `aead_algo`, `fec_algo`, or
   ...
   reader-side cap in v0.31. Parse KdfParams only after `kdf_algo` is
...
In v0.31, archive parameters guarantee at least one payload parity
...
end. For a clean v0.31 core volume, after the reader has skipped any
...
be zero in v0.31 archives. Readers MUST reject blocks with reserved flag
bits set.
...
No part of the v0.31 write path needs to revisit a closed S3 part or to
...
writing final volumes. There is no conforming v0.31 mode that round-robins
...
### 20.1 Padding overhead (v0.31 unchanged from v0.15)
...
document uses `format_version = 1` and `volume_format_rev = 31`. Readers
...
`volume_format_rev = 31`; accepting earlier draft revisions requires an
...
implementation claims conformance to this v0.31 draft, incompatible
...
### 28.1 Test corpus additions through v0.31

Additional v0.31 cases:
...
- **CryptoHeader canonical offset**: set `crypto_header_offset` to values
  ...
  with padding bytes before the CryptoHeader; verify v0.31 readers reject
  instead of accepting an unclaimed gap.
- **Volume format revision**: create archives with `volume_format_rev`
  below, equal to, and above 29; verify v0.31-only readers accept only 30
...
- **CryptoHeader Extension TLVs**: with a valid HMAC, inject an unknown
  ...
  verify v0.31 readers ignore it and continue parsing. Repeat with the
...
- **Compression registry enforcement**: mutate CryptoHeader to
  `compression_algo = None`; verify v0.31 readers reject it before
...
1. Produces archives whose write sequence is strictly forward
   ...
   `format_version = 1`, `volume_format_rev = 31`, every fixed magic
...
35. Emits v0.31 bootstrap sidecars as packed sequences with no padding,
...
## Appendix A: All changes from v0.30 -> v0.31

| Section | Change |
|---|---|
| §8 / §17.1 / §23 / §28.1 / §29 | `volume_format_rev` is bumped to 31 |
| §12.3 / §19.1 / §28.1 / §29 | Clarifies sidecar ManifestFooter instances: volume-0 byte equivalence is allowed when fields and HMAC inputs match; copying a nonzero-volume footer and mutating `volume_index` after HMAC remains invalid |
| §1 / §3 / §17.3 / §21 / §22 / §28.1 / §29 | Defines sequential-extraction live output as provisional until terminal ManifestFooter/VolumeTrailer authentication succeeds; default filesystem extractors must stage or quarantine before commit |
| §11 / §15.3 / §15.9 / §18 / §28.1 / §29 | Requires every present encrypted object to have at least one data block and rejects zero-data-block metadata/payload objects before FEC, AEAD, or decompression |

---

*End of v0.31 specification.**
