# tzap Archive Format Specification (v0.33)

| Field | Value |
|---|---|
| **Format version** | 1 |
| **Document version** | 0.33 / 2026-05-22.3 (interoperability fixes) |
| **Status** | Draft for review, no implementation yet |
| **Owner** | Frank Zhu |
| **Maintainers** | Frank Zhu |
| **Last updated** | 2026-05-22 |
| **Supersedes** | v0.1, v0.2, v0.3, v0.4, v0.5, v0.6, v0.7, v0.8, v0.9, v0.10, v0.11, v0.12, v0.13, v0.14, v0.15, v0.16, v0.17, v0.18, v0.19, v0.20, v0.21, v0.22, v0.23, v0.24, v0.25, v0.26, v0.27, v0.28, v0.29, v0.30, v0.31, v0.32 |
| **Superseded by** | None |
| **Conflict rule** | This document supersedes earlier tzap format drafts. If it conflicts with v0.1-v0.32 text, this v0.33 draft wins unless a later dated spec explicitly supersedes it. |
| **File extension** | `.tzap` (single-volume) / `.tzap.NNN` (multi-volume) |

## Changelog from v0.32

This revision resolves interoperability-significant findings in v0.32:
underspecified hash-prefix ordering, underspecified Argon2id parameters,
and underspecified GF(2^16) FEC symbol/block-size requirements.

1. **Draft visibility is bumped.** `volume_format_rev = 33` for this
   draft. (§8, §17.1, §23, §28.1, §29)
2. **Hash-prefix ordering is byte-stable.** All 8-byte SHA-256 prefixes
   used as sort keys, interval bounds, and lookup keys are compared in
   raw digest byte order, lexicographically, never as host-endian integer
   values. (§4, §15.1, §15.4, §15.8, §28.1)
3. **Argon2id is made wire-interoperable.** The KDF profile now fixes the
   Argon2 version, output length, and empty secret/associated-data inputs
   so independent implementations derive identical `master_key` bytes.
   (§13.1, §28.1)
4. **ReedSolomonGF16 gains symbol-size constraints.** `block_size` must be
   even, readers reject odd block sizes, and GF(2^16) symbols serialize as
   little-endian 16-bit words inside each shard. (§9, §17.1, §18, §24,
   §28.1, §29)
...
a usable archive mode. Conformant v0.33 writers MUST set
`compression_algo = ZstdFramed`; Conformant v0.33 readers MUST reject
...
this draft, not usable archive modes. Conformant v0.33 writers MUST set
`fec_algo = ReedSolomonGF16`; Conformant v0.33 readers MUST reject any
...
- `pad_len = 0` is not valid in v0.33. The extra block in the exact-fit
...
stream), conforming v0.33 writers MUST use `stripe_width = 1`,
...
struct VolumeHeader {
    ...
    volume_format_rev:        u16,       // 33 for this draft
...
parameters.
Readers MUST reject `compression_algo = None` in v0.33; payload and
...
known extensions are single-valued in v0.33; writers MUST NOT emit the
...
| ~~`0x0004`~~ | ~~`[u8; 32]`~~ | **Forbidden in v0.33.** The tar-stream content hash is encrypted inside IndexRoot. Writers MUST NOT emit this extension; readers MUST reject it if present. |
| `0x0005` | UTF-8 | Locale tag for filenames |
| ~~`0x0006`~~ | ~~bytes~~ | **Forbidden in v0.33; moved to encrypted metadata.** A writer setting `has_dictionary = 1` declares that IndexRoot locates a dictionary-object extent (§15.2). Writers MUST NOT emit this extension; readers MUST reject it if present. |
...
struct BlockRecord {
    ...
    flags:         u8,               // bit 0: last data block of encrypted object
                                     // bits 1..7: reserved; MUST be zero in v0.33
    _reserved:     [u8; 2],
...
MUST reject a BlockRecord with any reserved flag bit set; in v0.33 this
...
Completed v0.33 writers MUST set `is_authoritative = 1` in every closed
...
ManifestFooter and VolumeTrailer in v0.33.
...
struct BootstrapSidecarHeader {
    ...
    flags:                       u32,       // bit 0: ManifestFooter present
                                             ...
                                             // bits 3..31: reserved; MUST be zero in v0.33
...
be zero in v0.33; readers MUST reject the sidecar before trusting any
...
ManifestFooter HMAC and `archive_uuid`/`session_id`, MUST reject a v0.33
...
No padding, extension bytes, or unclaimed gaps are permitted in a v0.33
...
non-critical by a future version; for v0.33, unknown flag bits are a hard
error.
...
payload envelope sequence has no gaps. Because v0.33 stores only
...
For every encrypted object in v0.33, `encrypted_size` is the total
...
`FileEntry.flags` is reserved in v0.33. Writers MUST set it to zero, and
...
- when the same `frame_index` or `envelope_index` appears in more than
  ...
  version, this is equivalent to byte-identical row encoding in v0.33.
...
1. Read VolumeHeader at offset 0; verify CRC.
   ...
   by the reader. A reader claiming only v0.33 conformance rejects
   `volume_format_rev != 33`. Reject if `VolumeHeader.stripe_width = 0`
...
3. Reject unknown `compression_algo`, `aead_algo`, `fec_algo`, or
   ...
   its active reader-side cap in v0.33. Parse KdfParams only after `kdf_algo` is
...
In v0.33, archive parameters guarantee at least one payload parity
...
end. For a clean v0.33 core volume, after the reader has skipped any
...
`ReedSolomonGF16` is a systematic object-local shard codec over
GF(2^16). Each data or parity shard is exactly `BLOCK_SIZE` bytes, and
...
byte order. The object-local shard indexes are `0 .. D - 1` for data
shards in encrypted-byte order and `D .. D + P - 1` for parity shards in
parity-shard order; the on-disk BlockRecord order remains all data
blocks first, then all parity blocks. A codec that uses a GF(2^8)
matrix, a different GF(2^16) symbol byte order, a non-systematic code,
or a different parity-shard ordering under `FecAlgo::ReedSolomonGF16` is
not wire-compatible with this draft. Implementations MUST match the
reference parity vectors for this `FecAlgo` before claiming write or
repair compatibility; an implementation that chooses a different
generator matrix or parity convention needs a different FEC algorithm ID
rather than reusing `ReedSolomonGF16`.
...
be zero in v0.33 archives. Readers MUST reject blocks with reserved flag
bits set.
...
No part of the v0.33 write path needs to revisit a closed S3 part or to
...
writing final volumes. There is no conforming v0.33 mode that round-robins
...
### 20.1 Padding overhead (v0.33 unchanged from v0.15)
...
document uses `format_version = 1` and `volume_format_rev = 33`. Readers
...
`volume_format_rev = 33`; accepting earlier draft revisions requires an
...
implementation claims conformance to this v0.33 draft, incompatible
...
### 28.1 Test corpus additions through v0.33

Additional v0.33 cases:
...
- **Argon2id profile vectors**: fixed passphrase bytes, salt, `t_cost`,
  ...
  not accept PHC-string defaults or alternate Argon2 versions as
  equivalent.
- **ReedSolomonGF16 wire profile vectors**: fixed even `block_size`,
  ordered data shards, `D`, and `P` with expected parity BlockRecord
  payload bytes. Include vectors that distinguish little-endian 16-bit
  symbol serialization, data-first then parity ordering, and the
  registered GF(2^16) profile from GF(2^8) Reed-Solomon codecs. Also set
  an odd `block_size` and verify readers reject before FEC repair.
...
- **Volume format revision freshness**: create archives with
  `volume_format_rev` below, equal to, and above 33; verify v0.33-only
  readers accept only 33 and reject older or newer revisions.
...
- **CryptoHeader canonical offset**: set `crypto_header_offset` to values
  ...
  with padding bytes before the CryptoHeader; verify v0.33 readers reject
  instead of accepting an unclaimed gap.
- **Volume format revision**: create archives with `volume_format_rev`
  below, equal to, and above 33; verify v0.33-only readers accept only 33
...
- **CryptoHeader Extension TLVs**: with a valid HMAC, inject an unknown
  ...
  verify v0.33 readers ignore it and continue parsing. Repeat with the
...
- **Compression registry enforcement**: mutate CryptoHeader to
  `compression_algo = None`; verify v0.33 readers reject it before
...
1. Produces archives whose write sequence is strictly forward
   ...
   `format_version = 1`, `volume_format_rev = 33`, every fixed magic
...
14. Includes directory hint shards when
    ...
    counters for directory-hint shard objects.
...
20. Sets `compression_algo = ZstdFramed`, sets
   ...
   boundary. Emits KDF parameter payloads exactly as §13.1 specifies,
    including
    `m_cost_kib ≥ 8 × parallelism` for Argon2id, and feeds Argon2id the
    exact UTF-8 bytes of the NFC-normalized caller passphrase without
...
35. Emits v0.33 bootstrap sidecars as packed sequences with no padding,
...
9. Validates IndexRoot, IndexShard, and DirectoryHintTable magic fields,
   ...
   FrameEntry and sparse EnvelopeEntry semantics, stored path/hash
   bindings, and non-overlap before allocation or indexing. For local
...
## Appendix A: All changes from v0.32 -> v0.33

| Section | Change |
|---|---|
| §8 / §17.1 / §23 / §28.1 / §29 | `volume_format_rev` is bumped to 33 |
| §4 / §15.1 / §15.4 / §15.8 / §28.1 | Defines `[u8; 8]` SHA-256 prefix ordering as raw bytewise lexicographic order |
| §13.1 / §28.1 | Fixes the Argon2id profile: version 0x13, 32-byte output, empty secret, and empty associated data |
| §9 / §17.1 / §18 / §24 / §28.1 / §29 | Requires even `block_size` for GF(2^16), defines little-endian 16-bit symbol serialization, and adds FEC wire-profile vectors |

---

*End of v0.33 specification.*
