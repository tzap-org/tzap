# tzap Archive Format Specification (v0.24)

| Field | Value |
|---|---|
| **Format version** | 1 |
| **Document version** | 0.24 / 2026-05-21.1 (critical extension behavior fix) |
| **Status** | Draft for review, no implementation yet |
| **Owner** | Frank Zhu |
| **Maintainers** | Frank Zhu |
| **Last updated** | 2026-05-21 |
| **Supersedes** | v0.1, v0.2, v0.3, v0.4, v0.5, v0.6, v0.7, v0.8, v0.9, v0.10, v0.11, v0.12, v0.13, v0.14, v0.15, v0.16, v0.17, v0.18, v0.19, v0.20, v0.21, v0.22, v0.23 |
| **Superseded by** | None |
| **Conflict rule** | This document supersedes earlier tzap format drafts. If it conflicts with v0.1-v0.23 text, this v0.24 draft wins unless a later dated spec explicitly supersedes it. |
| **File extension** | `.tzap` (single-volume) / `.tzap.NNN` (multi-volume) |

## Changelog from v0.23

This revision resolves final implementation-readiness findings in v0.23:
critical CryptoHeader extension handling and one remaining ambiguity in
CryptoHeader parsing.

1. **Draft visibility is bumped.** `volume_format_rev = 24` for this
   draft. (§8, §17.1, §23)
2. **Directory-hint completeness is defined.** Writers must build exact
   directory-to-shard-row maps, and full-archive `verify` must recompute
   and compare those maps before treating hints as completeness-proven.
   (§15.8, §15.9, §28.1, §29)
3. **Directory-prefix duplicate-path behavior matches path lookup.**
   Prefix extraction uses final-view "last entry wins" semantics by
   default; all-occurrence output is explicit history mode. (§15.8, §29)
4. **CryptoHeader minimum length includes the TLV terminator.** The
   smallest valid header includes fixed fields, raw KdfParams, the
   6-byte terminator TLV, and HMAC. (§9, §28.1)
5. **Sequential zstd parsing is explicit without FrameEntry metadata.**
   Sequential readers parse each depadded envelope plaintext as a
   concatenated zstd-frame stream and reject trailing or truncated frame
   bytes. (§17.3, §28.1, §29)
6. **FileEntry rows are bound to decoded tar member groups.** Full
   archive verification now checks that FileEntry extents exactly match
   the parsed tar member group sequence, and extraction checks the main
   tar entry path/size against the FileEntry before writing. (§15.6,
   §15.7, §15.9, §17.2, §28.1, §29)
7. **Critical extension semantics are explicit.** v0.24 readers MUST
   reject unknown critical CryptoHeader extensions and MUST ignore unknown
   non-critical CryptoHeader extensions.
...
a usable archive mode. Conformant v0.24 writers MUST set
`compression_algo = ZstdFramed`; Conformant v0.24 readers MUST reject
...
this draft, not usable archive modes. Conformant v0.24 writers MUST set
`fec_algo = ReedSolomonGF16`; Conformant v0.24 readers MUST reject any
...
- `pad_len = 0` is not valid in v0.24. The extra block in the exact-fit
...
stream), conforming v0.24 writers MUST use `stripe_width = 1`,
...
struct VolumeHeader {
    ...
    volume_format_rev:        u16,       // 24 for this draft
...
parse Extension TLVs.
For each extension TLV, readers MUST define `ext_tag = tag & 0x7FFF` and
`is_critical = (tag & 0x8000) != 0`. The high bit is reserved for the
critical bit and must be masked from `ext_tag` when validating known tags.
Readers MUST:
1. Reject unknown `ext_tag` values when `is_critical = true` (critical
   extension; hard error).
2. Ignore unknown non-critical `ext_tag` values when `is_critical = false`
   unless a future version reserves that tag.
3. Reject any extension whose payload length is out-of-range for its
   known type in this draft.
Readers MUST reject a CryptoHeader whose TLV list does not terminate
before `length - 32`, or whose reserved bytes are non-zero.
...
parameters.
Readers MUST reject `compression_algo = None` in v0.24; payload and
...
| ~~`0x0004`~~ | ~~`[u8; 32]`~~ | **Forbidden in v0.24.** The tar-stream content hash is encrypted inside IndexRoot. Writers MUST NOT emit this extension; readers MUST reject it if present. |
...
struct BlockRecord {
    ...
    flags:         u8,               // bit 0: last data block of encrypted object
                                     // bits 1..7: reserved; MUST be zero in v0.24
    _reserved:     [u8; 2],
...
MUST reject a BlockRecord with any reserved flag bit set; in v0.24 this
...
Completed v0.24 writers MUST set `is_authoritative = 1` in every closed
...
ManifestFooter and VolumeTrailer in v0.24.
...
struct BootstrapSidecarHeader {
    ...
    flags:                       u32,       // bit 0: ManifestFooter present
                                             ...
                                             // bits 3..31: reserved; MUST be zero in v0.24
...
be zero in v0.24; readers MUST reject the sidecar before trusting any
...
ManifestFooter HMAC and `archive_uuid`/`session_id`, MUST reject a v0.24
...
No padding, extension bytes, or unclaimed gaps are permitted in a v0.24
...
non-critical by a future version; for v0.24, unknown flag bits are a hard
error.
...
For every encrypted object in v0.24, `encrypted_size` is the total
...
`FileEntry.flags` is reserved in v0.24. Writers MUST set it to zero, and
...
- when the same `frame_index` or `envelope_index` appears in more than
  ...
  version, this is equivalent to byte-identical row encoding in v0.24.
...
1. Read VolumeHeader at offset 0; verify CRC.
   ...
   only v0.24 conformance rejects `volume_format_rev != 24`.
...
3. Parse KdfParams; prompt for passphrase or load keyfile. If
   ...
   `length - 32`. Reject unknown algorithm IDs, unknown critical
   CryptoHeader extensions, `compression_algo != ZstdFramed`,
   `fec_algo != ReedSolomonGF16`, invalid boolean `has_dictionary`,
   `volume_loss_tolerance >= stripe_width`, `bit_rot_buffer_pct > 100`,
   or zero data-shard class maxima in v0.24.
...
In v0.24, archive parameters guarantee at least one payload parity
...
end. For a clean v0.24 core volume, after the reader has skipped any
...
be zero in v0.24 archives. Readers MUST reject blocks with reserved flag
bits set.
...
No part of the v0.24 write path needs to revisit a closed S3 part or to
...
writing final volumes. There is no conforming v0.24 mode that round-robins
...
### 20.1 Padding overhead (v0.24 unchanged from v0.15)
...
document uses `format_version = 1` and `volume_format_rev = 24`. Readers
...
`volume_format_rev = 24`; accepting earlier draft revisions requires an
...
implementation claims conformance to this v0.24 draft, incompatible
...
### 28.1 Test corpus additions for v0.24
...
- **Volume format revision**: create archives with `volume_format_rev`
  below, equal to, and above 24; verify v0.24-only readers accept only
  24 and reject older or newer revisions.
...
- **Critical extension handling**: inject an unknown non-critical extension
  tag `0x0009` with valid length and payload; verify v0.24 readers ignore
  it and continue parsing. Repeat with the same unknown extension as
  critical `0x8009`; verify readers reject with a hard error.
...
- **KDF parameter caps**: Argon2id salt lengths below 8 and above 64,
  ...
  extension parsing or KDF execution.
...
- **Compression registry enforcement**: mutate CryptoHeader to
  `compression_algo = None`; verify v0.24 readers reject it before
...
1. Produces archives whose write sequence is strictly forward
   ...
   `format_version = 1` and `volume_format_rev = 24`.
...
35. Emits v0.24 bootstrap sidecars as packed sequences with no padding,
...
15. Rejects CryptoHeader extension tag `0x0004`; rejects unknown
   critical CryptoHeader extension tags; ignores unknown non-critical
   extensions.
...
18. Rejects unknown algorithm IDs, `compression_algo != ZstdFramed`,
    ...
    KdfParams payload and the required TLV terminator fit inside
    CryptoHeader before the HMAC. It then
...
23. Rejects non-zero `FileEntry.flags`, unsafe archive paths, and
    ...
    computed with checked unsigned 64-bit-or-wider arithmetic.
...
## Appendix A: All changes from v0.23 → v0.24

| Section | Change |
|---|---|
| §8 / §17.1 / §23 / §28.1 / §29 | `volume_format_rev` is bumped to 24 and the stale revision-acceptance test is corrected |
| §9 / §28.1 | CryptoHeader minimum length now includes the required 6-byte Extension TLV terminator |
| §9.1 / §29 | Unknown critical extensions are hard errors; unknown non-critical extensions are ignored |
| §15.8 / §15.9 / §28.1 / §29 | Directory hints are exact maps; full-archive `verify` recomputes and compares them before treating hints as completeness-verified |
| §15.8 / §29 | Directory-prefix extraction uses final-view duplicate-path semantics by default and exposes earlier occurrences only through explicit history/listing mode |
| §17.3 / §28.1 / §29 | Sequential extraction parses each depadded envelope plaintext as a concatenated zstd-frame stream without relying on FrameEntry metadata |
| §15.6 / §15.7 / §15.9 / §17.2 / §28.1 / §29 | FileEntry rows are bound to decoded tar member groups by exact extent, normalized path, and logical payload size |
| §28.1 / §29 | v0.24 tests and conformance checklist expanded |

---

*End of v0.24 specification.*
