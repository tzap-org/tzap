# tzap Archive Format Specification (v0.22)

| Field | Value |
|---|---|
| **Format version** | 1 |
| **Document version** | 0.22 / 2026-05-21.3 (self-reviewed draft after sequential-state and duplicate-path cleanup) |
| **Status** | Draft for review, no implementation yet |
| **Owner** | Frank Zhu |
| **Maintainers** | Frank Zhu |
| **Last updated** | 2026-05-21 |
| **Supersedes** | v0.1, v0.2, v0.3, v0.4, v0.5, v0.6, v0.7, v0.8, v0.9, v0.10, v0.11, v0.12, v0.13, v0.14, v0.15, v0.16, v0.17, v0.18, v0.19, v0.20, v0.21 |
| **Superseded by** | None |
| **Conflict rule** | This document supersedes earlier tzap format drafts. If it conflicts with v0.1-v0.21 text, this v0.22 draft wins unless a later dated spec explicitly supersedes it. |
| **File extension** | `.tzap` (single-volume) / `.tzap.NNN` (multi-volume) |

## Changelog from v0.21

This revision resolves follow-up review findings that could make
independent readers diverge around sequential corruption, directory
hints, duplicate tar paths, and short CryptoHeaders.

1. **Draft visibility is bumped.** `volume_format_rev = 22` for this
   draft. (§8, §17.1, §23)
2. **Sequential optional-parity CRC failure is fail-fast.** A pure
   sequential reader MUST abort on a CRC-failed payload data block unless
   archive parameters guarantee at least one parity BlockRecord for every
   payload envelope and the next trusted block starts that parity run.
   Otherwise the last-data flag might be in the failed block and the
   remaining stream cannot be framed safely. (§17.3, §28.1, §29)
3. **Directory hints now store shard-row indexes, not shard IDs.** The
   u32 hint array points into the canonical IndexRoot ShardEntry table;
   ShardEntry.shard_index remains a u64 stable object identifier, not a
   row position. (§15.8, §15.9, §28.1, §29)
4. **Duplicate tar paths have deterministic random-access semantics.**
   FileEntry rows for identical normalized paths are ordered by tar-stream
   occurrence; default path lookup returns the last occurrence, matching
   normal tar overwrite behavior. (§15.6, §15.7, §28.1, §29)
5. **CryptoHeader length validation includes KdfParams.** Readers must
   prove the header is long enough for the selected KDF parameter payload
   before parsing extension TLVs. (§9, §13.1, §17.1, §28.1, §29)
6. **Zstd frame validity is explicit.** Each FrameEntry slice must parse
   and decode as exactly one complete zstd frame; decompression failure is
   a hard archive error. (§15.3, §15.9, §17.2, §28.1)
7. **Directory-hint range arithmetic is nailed down.** Shard-row list
   additions and byte-size multiplications use checked unsigned
   64-bit-or-wider arithmetic before indexing. (§15.8, §15.9)
8. **Bootstrap and trailer wording is tightened.** Bootstrap sidecar
   reserved bytes are explicitly zero, BlockRecord CRC wording names the
   covered fields, and trailer `block_count` consistency is restated for
   trailing-garbage recovery. (§10, §12.3, §17.1)
...
a usable archive mode. Conformant v0.22 writers MUST set
`compression_algo = ZstdFramed`; conformant v0.22 readers MUST reject
...
this draft, not usable archive modes. Conformant v0.22 writers MUST set
`fec_algo = ReedSolomonGF16`; conformant v0.22 readers MUST reject any
...
- `pad_len = 0` is not valid in v0.22. The extra block in the exact-fit
...
stream), conforming v0.22 writers MUST use `stripe_width = 1`,
...
struct VolumeHeader {
    ...
    volume_format_rev:        u16,       // 22 for this draft
...
payload, and the HMAC. At minimum, a candidate header must contain
`sizeof(CryptoHeaderFixed) + 2 + 32` bytes so the KdfParams `algo_tag`
can be read safely; after `kdf_algo` is known, readers MUST prove the
complete KdfParams payload indicated by §13.1 fits before
`length - 32`, and only then parse Extension TLVs. Readers MUST reject a
CryptoHeader whose TLV list does not terminate before `length - 32`, or
whose reserved bytes are non-zero.
...
parameters.
Readers MUST reject `compression_algo = None` in v0.22; payload and
...
| ~~`0x0004`~~ | ~~`[u8; 32]`~~ | **Forbidden in v0.22.** The tar-stream content hash is encrypted inside IndexRoot. Writers MUST NOT emit this extension; readers MUST reject it if present. |
...
struct BlockRecord {
    ...
    flags:         u8,               // bit 0: last data block of encrypted object
                                     // bits 1..7: reserved; MUST be zero in v0.22
    _reserved:     [u8; 2],
...
MUST reject a BlockRecord with any reserved flag bit set; in v0.22 this
...
Completed v0.22 writers MUST set `is_authoritative = 1` in every closed
...
ManifestFooter and VolumeTrailer in v0.22.
...
struct BootstrapSidecarHeader {
    ...
    flags:                       u32,       // bit 0: ManifestFooter present
                                             ...
                                             // bits 3..31: reserved; MUST be zero in v0.22
...
be zero in v0.22; readers MUST reject the sidecar before trusting any
...
ManifestFooter HMAC and `archive_uuid`/`session_id`, MUST reject a v0.22
...
No padding, extension bytes, or unclaimed gaps are permitted in a v0.22
...
non-critical by a future version; for v0.22, unknown flag bits are a hard
error.
...
For every encrypted object in v0.22, `encrypted_size` is the total
...
`FileEntry.flags` is reserved in v0.22. Writers MUST set it to zero, and
...
row indexes for shards that contain direct children or descendants of
that directory. They are
...
- when the same `frame_index` or `envelope_index` appears in more than
  ...
  version, this is equivalent to byte-identical row encoding in v0.22.
...
1. Read VolumeHeader at offset 0; verify CRC.
   ...
   only v0.22 conformance rejects `volume_format_rev != 22`.
...
3. Parse KdfParams; prompt for passphrase or load keyfile. If
   ...
   payload fits before `length - 32`. If `kdf_algo == Argon2id`, first
   verify that the 16-byte fixed Argon2id KdfParams prefix fits, then
   reject if `t_cost`, `m_cost_kib`, `parallelism`, or `salt_length`
   exceed reader caps or if `salt_length` is outside 8..64 bytes,
   `parallelism = 0`, `m_cost_kib < 8 × parallelism`, or the full
   `16 + salt_length` KdfParams payload does not fit before
   `length - 32`. Reject unknown algorithm IDs, `compression_algo !=
   ZstdFramed`, `fec_algo != ReedSolomonGF16`, invalid boolean
   `has_dictionary`, `volume_loss_tolerance >= stripe_width`,
   `bit_rot_buffer_pct > 100`, or zero data-shard class maxima in v0.22.
...
In v0.22, archive parameters guarantee at least one payload parity
...
end. For a clean v0.22 core volume, after the reader has skipped any
...
be zero in v0.22 archives. Readers MUST reject blocks with reserved flag
bits set.
...
No part of the v0.22 write path needs to revisit a closed S3 part or to
...
writing final volumes. There is no conforming v0.22 mode that round-robins
...
### 20.1 Padding overhead (v0.22 unchanged from v0.15)
...
document uses `format_version = 1` and `volume_format_rev = 22`. Readers
...
`volume_format_rev = 22`; accepting earlier draft revisions requires an
...
implementation claims conformance to this v0.22 draft, incompatible
...
### 28.1 Test corpus additions for v0.22
...
- **Volume format revision**: create archives with `volume_format_rev`
  below, equal to, and above 22; verify v0.22-only readers accept only
...
- **KDF parameter caps**: Argon2id salt lengths below 8 and above 64,
  ...
  short Argon2id prefix, or an Argon2id salt shorter than `salt_length`;
  verify readers reject before extension parsing or KDF execution.
...
- **Compression registry enforcement**: mutate CryptoHeader to
  `compression_algo = None`; verify v0.22 readers reject it before
...
1. Produces archives whose write sequence is strictly forward
   ...
   `format_version = 1` and `volume_format_rev = 22`.
...
35. Emits v0.22 bootstrap sidecars as packed sequences with no padding,
...
18. Rejects unknown algorithm IDs, `compression_algo != ZstdFramed`,
    ...
    `block_size` values above configured reader caps. Before parsing
    Extension TLVs or running the KDF, verifies that the selected
    KdfParams payload fits inside CryptoHeader before the HMAC. It then
    treats the validated `chunk_size` and `expected_volume_size` as
    advisory metadata only; FrameEntry, EnvelopeEntry, and authenticated
...
29. Rejects `dictionary_data_block_count = 0` with any non-zero IndexRoot
    ...
    values, invalid DirectoryHintShardEntry ordering, shard-row-index
    values outside the IndexRoot ShardEntry table, invalid shard-list
...
## Appendix A: All changes from v0.21 → v0.22

| Section | Change |
|---|---|
| §17.3 / §28.1 / §29 | Sequential extraction aborts on an unauthenticated CRC-failed payload data block unless parity is guaranteed and the next trusted block begins the parity run |
| §15.8 / §15.9 / §28.1 / §29 | Directory hints store ShardEntry row indexes, not stable `shard_index` IDs |
| §15.6 / §15.7 / §17.2 / §28.1 / §29 | Duplicate normalized tar paths are ordered by `tar_member_group_start`; default random access returns the last occurrence |
| §9 / §13.1 / §17.1 / §28.1 / §29 | CryptoHeader length validation proves the selected KdfParams payload fits before parsing TLVs or running KDF |
| §15.3 / §15.9 / §17.2 / §28.1 / §29 | FrameEntry slices must decode as exactly one complete zstd frame |
| §15.8 / §15.9 / §28.1 | Directory-hint shard-row range additions and `×4` byte-size calculations use checked unsigned 64-bit-or-wider arithmetic |
| §10 / §12.3 / §17.1 / §28.1 / §29 | BlockRecord CRC wording, BootstrapSidecarHeader reserved-byte checks, and trailer `block_count` consistency wording are clarified |
| §28.1 / §29 | v0.22 tests and conformance checklist expanded |

---

*End of v0.22 specification.*
