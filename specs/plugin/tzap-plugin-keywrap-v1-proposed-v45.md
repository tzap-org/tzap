# tzap-plugin-keywrap-v1 - proposed v45 recipient key wrapping

Specification for the first recipient key-wrap plugin profile for tzap v45
archives with `KdfAlgo::RecipientWrap`.

Status: proposed v45 implementation target for the RecipientWrap key-wrap
profile. The v45 implementation exposes a dedicated `tzap-plugin-keywrap` crate
and CLI `--recipient-cert` / `--recipient-key` key-source flags for the initial
single-recipient workflow. Targets `specs/tzap-format-revisedv45.md`, especially
...
Those are core v45, signing plugin, deployment, or future-plugin
responsibilities.
...
2. Parsed RecipientWrap KdfParams from the v45 spec.
...
### 3.1 Revision Dispatch

This document defines `profile_id = 0x0001` for core revision 45 only.
Implementations that also support revision 44 MUST dispatch on the validated
core `volume_format_rev` before constructing the HPKE context. For revision 45,
the AAD domain is exactly `tzap-keywrap-master-key-v45\0` as specified in §9.
A reader MUST NOT retry the revision-44 AAD after HPKE authentication failure;
cross-revision fallback is non-conforming. The profile ID and payload version
remain unchanged because `volume_format_rev` is included in
`key_wrap_context_digest` and core performs revision dispatch before plugin use.
...
aad  = "tzap-keywrap-master-key-v45\0" || key_wrap_context_digest
```
...
9. Certificate policy rejection: a record targets a local private key but
   ...
   `CertificatePolicyRejected` without attempting unwrap.
10. Table and record binding: changing common table framing or table bytes
    without updating KdfParams causes core's revision-45
    `key_wrap_table_digest` check to fail. Changing recipient identity or profile
    payload bytes causes profile validation or HPKE Open to fail. A forged
    table/header combination that still yields a candidate key MUST fail
    `CryptoHeader.header_hmac` before any archive-object decryption.
11. Signing separation: a RootAuth signing certificate does not satisfy recipient
    ...
    are present.
12. Cross-revision separation: a revision-44 recipient payload/AAD combination
    does not open as revision 45, and a revision-45 implementation does not retry
    the revision-44 AAD after authentication failure.
