# tzap-plugin-signing-v6 - proposed v45 Ed25519 root authenticator

Specification for the optional Ed25519 signing authenticator profile for tzap
v45 root authentication.

Status: v45 implementation target for the current core helper API. Targets
...
Those are core v45 or RecipientWrap key-wrap profile responsibilities.
...
This document defines `authenticator_id = 0x0002` for core revision 45 only.
...
authenticator ID and Ed25519 authenticator-value version remain unchanged; the
...
the v45 public no-key observation path. It proves only that a trusted key signed
...
1. `RootAuthContentVerified`: matching caller key, valid full v45 RootAuth
