# tzap-plugin-x509-chain-v2 - proposed v45 X.509 root authenticator

Specification for the optional X.509 certificate-chain authenticator profile for
tzap v45 root authentication.

Status: v45 implementation target for `tzap-plugin-signing::x509_chain` and
matching tzap CLI X.509 RootAuth flags. Targets
`specs/tzap-format-revisedv45.md` and `root_auth_spec_id` as the 20 ASCII bytes
`tzap-root-auth-v0.45` followed by four zero bytes.
This v2 document keeps the v1 X.509 authenticator wire profile and updates its
core contract for v45 archive-root inputs.
...
Those are core v45, deployment, or RecipientWrap key-wrap profile
responsibilities.
...
1. `RootAuthFooterV1.root_auth_spec_id` is the exact 24-byte value consisting
   of the 20 ASCII bytes `tzap-root-auth-v0.45` followed by four zero bytes.
2. The archive has `VolumeHeader.volume_format_rev = 45`.
3. Core has wire-validated `RootAuthFooterV1`.
4. Core has recomputed `archive_root` according to the v45 spec.
...
This profile is **protection-mode independent** (v45 §5.1). It signs and verifies
...
For v45 recipient-wrap archives (`kdf_algo = RecipientWrap`), this profile is
...
### 2.1 Revision Dispatch

This document defines `authenticator_id = 0x0003` for core revision 45 only.
Implementations that also support revision 44 MUST dispatch on the
wire-validated `RootAuthFooterV1.root_auth_spec_id` and
`volume_format_rev` before invoking this profile. They MUST NOT accept the
v0.44 root-auth spec ID for a revision-45 archive, and they MUST NOT retry a
different revision's signing input after verification failure. The
authenticator ID and X.509 authenticator-value version remain unchanged; the
core revision binding is provided by the validated spec ID and revision fields.
...
The final `authenticator_value_length` MUST still satisfy the v45
...
- the recomputed v45 `archive_root` matched the stored footer commitment;
...
`RootAuthContentVerified` is available only when core has completed full v45
...
the v45 public no-key observation path. It proves only that a certificate chain
...
1. Core validates and recomputes v45 root-auth inputs.
...
accompanied by the v45 public no-key diagnostics.

## 13. Partial Operations

This profile MUST NOT return a verified outcome for partial/random single-file
extraction unless core has already completed the full v45 verification required
for that outcome.
...
1. `RootAuthContentVerified`: matching trusted root, valid full v45 RootAuth
