# tzap Archive Format Specification (v0.29)

| Field | Value |
|---|---|
| **Format version** | 1 |
| **Document version** | 0.29 / 2026-05-21.5 (implementation-readiness fixes) |
| **Status** | Draft for review, no implementation yet |
| **Owner** | Frank Zhu |
| **Maintainers** | Frank Zhu |
| **Last updated** | 2026-05-21 |
| **Supersedes** | v0.1, v0.2, v0.3, v0.4, v0.5, v0.6, v0.7, v0.8, v0.9, v0.10, v0.11, v0.12, v0.13, v0.14, v0.15, v0.16, v0.17, v0.18, v0.19, v0.20, v0.21, v0.22, v0.23, v0.24, v0.25, v0.26, v0.27, v0.28 |
| **Superseded by** | None |
| **Conflict rule** | This document supersedes earlier tzap format drafts. If it conflicts with v0.1-v0.28 text, this v0.29 draft wins unless a later dated spec explicitly supersedes it. |
| **File extension** | `.tzap` (single-volume) / `.tzap.NNN` (multi-volume) |

## Changelog from v0.28

This revision resolves implementation-readiness findings in v0.28:
contradictory shard-local EnvelopeEntry/FrameEntry validation, missing
full-verify enforcement for required directory hints, and underspecified
tar directory path canonicalization.

1. **Draft visibility is bumped.** `volume_format_rev = 29` for this
   draft. (§8, §17.1, §23, §28.1, §29)
2. **Shard-local envelope validation no longer requires unrelated
   frames.** `EnvelopeEntry.first_frame_index` / `frame_count` remain
   global envelope metadata, but an IndexShard-local FrameEntry table is
   still only the exact set referenced by that shard's FileEntries.
   Local validation checks referenced slices and non-overlap; full
   archive verification unions distinct FrameEntry rows before requiring
   complete envelope frame coverage. (§15.3, §15.9, §28.1, §29)
3. **Required directory hints are verify-visible.** Normal readers may
   fall back to scanning all shards when hints are unavailable, but
   full-archive `verify` rejects archives whose `file_count` exceeds the
   required-hint threshold without a complete exact hint map. (§15.8,
   §15.9, §28.1, §29)
4. **Tar directory path canonicalization is explicit.** FileEntry paths
   never contain trailing slashes, while tar directory main-entry names
   may carry one conventional trailing slash that is stripped only for
   normalized path comparison and hint derivation. Other empty
   components remain malformed. (§16, §28.1, §29)
...
a usable archive mode. Conformant v0.29 writers MUST set
`compression_algo = ZstdFramed`; Conformant v0.29 readers MUST reject
...
this draft, not usable archive modes. Conformant v0.29 writers MUST set
`fec_algo = ReedSolomonGF16`; Conformant v0.29 readers MUST reject any
...
- `pad_len = 0` is not valid in v0.29. The extra block in the exact-fit
...
stream), conforming v0.29 writers MUST use `stripe_width = 1`,
...
struct VolumeHeader {
    ...
    volume_format_rev:        u16,       // 29 for this draft
...
parameters.
Readers MUST reject `compression_algo = None` in v0.29; payload and
...
known extensions are single-valued in v0.29; writers MUST NOT emit the
...
| ~~`0x0004`~~ | ~~`[u8; 32]`~~ | **Forbidden in v0.29.** The tar-stream content hash is encrypted inside IndexRoot. Writers MUST NOT emit this extension; readers MUST reject it if present. |
| `0x0005` | UTF-8 | Locale tag for filenames |
| ~~`0x0006`~~ | ~~bytes~~ | **Forbidden in v0.29; moved to encrypted metadata.** A writer setting `has_dictionary = 1` declares that IndexRoot locates a dictionary-object extent (§15.2). Writers MUST NOT emit this extension; readers MUST reject it if present. |
...
struct BlockRecord {
    ...
    flags:         u8,               // bit 0: last data block of encrypted object
                                     // bits 1..7: reserved; MUST be zero in v0.29
    _reserved:     [u8; 2],
...
MUST reject a BlockRecord with any reserved flag bit set; in v0.29 this
...
Completed v0.29 writers MUST set `is_authoritative = 1` in every closed
...
ManifestFooter and VolumeTrailer in v0.29.
...
struct BootstrapSidecarHeader {
    ...
    flags:                       u32,       // bit 0: ManifestFooter present
                                             ...
                                             // bits 3..31: reserved; MUST be zero in v0.29
...
be zero in v0.29; readers MUST reject the sidecar before trusting any
...
ManifestFooter HMAC and `archive_uuid`/`session_id`, MUST reject a v0.29
...
No padding, extension bytes, or unclaimed gaps are permitted in a v0.29
...
non-critical by a future version; for v0.29, unknown flag bits are a hard
error.
...
payload envelope sequence has no gaps. Because v0.29 stores only
...
For every encrypted object in v0.29, `encrypted_size` is the total
...
`FileEntry.flags` is reserved in v0.29. Writers MUST set it to zero, and
...
- when the same `frame_index` or `envelope_index` appears in more than
  ...
  version, this is equivalent to byte-identical row encoding in v0.29.
...
1. Read VolumeHeader at offset 0; verify CRC.
   ...
   by the reader. A reader claiming only v0.29 conformance rejects
   `volume_format_rev != 29`. Reject if `VolumeHeader.stripe_width = 0`
...
3. Reject unknown `compression_algo`, `aead_algo`, `fec_algo`, or
   ...
   reader-side cap in v0.29. Parse KdfParams only after `kdf_algo` is
...
In v0.29, archive parameters guarantee at least one payload parity
...
end. For a clean v0.29 core volume, after the reader has skipped any
...
be zero in v0.29 archives. Readers MUST reject blocks with reserved flag
bits set.
...
No part of the v0.29 write path needs to revisit a closed S3 part or to
...
writing final volumes. There is no conforming v0.29 mode that round-robins
...
### 20.1 Padding overhead (v0.29 unchanged from v0.15)
...
document uses `format_version = 1` and `volume_format_rev = 29`. Readers
...
`volume_format_rev = 29`; accepting earlier draft revisions requires an
...
implementation claims conformance to this v0.29 draft, incompatible
...
### 28.1 Test corpus additions for v0.29
...
- **CryptoHeader canonical offset**: set `crypto_header_offset` to values
  ...
  with padding bytes before the CryptoHeader; verify v0.29 readers reject
  instead of accepting an unclaimed gap.
- **Volume format revision**: create archives with `volume_format_rev`
  below, equal to, and above 29; verify v0.29-only readers accept only 29
...
- **CryptoHeader Extension TLVs**: with a valid HMAC, inject an unknown
  ...
  verify v0.29 readers ignore it and continue parsing. Repeat with the
...
- **Compression registry enforcement**: mutate CryptoHeader to
  `compression_algo = None`; verify v0.29 readers reject it before
...
1. Produces archives whose write sequence is strictly forward
   ...
   `format_version = 1`, `volume_format_rev = 29`, every fixed magic
...
35. Emits v0.29 bootstrap sidecars as packed sequences with no padding,
...
## Appendix A: All changes from v0.28 → v0.29

| Section | Change |
|---|---|
| §8 / §17.1 / §23 / §28.1 / §29 | `volume_format_rev` is bumped to 29 |
| §15.3 / §15.9 / §28.1 / §29 | Clarifies that shard-local FrameEntry tables remain exact local subsets even when local EnvelopeEntry rows describe a larger global envelope frame range; whole-envelope frame coverage is checked after full-archive verification unions distinct global FrameEntry rows |
| §15.8 / §15.9 / §28.1 / §29 | Full-archive `verify` now rejects archives above `directory_hint_required_file_count` that omit required directory hints, while ordinary extraction may still fall back to scanning all shards with diagnostics |
| §16 / §28.1 / §29 | Defines conventional tar directory trailing-slash canonicalization for FileEntry path comparison and directory-hint derivation |

---

*End of v0.29 specification.*
