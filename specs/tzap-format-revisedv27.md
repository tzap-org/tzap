# tzap Archive Format Specification (v0.27)

| Field | Value |
|---|---|
| **Format version** | 1 |
| **Document version** | 0.27 / 2026-05-21.3 (implementation-readiness fixes) |
| **Status** | Draft for review, no implementation yet |
| **Owner** | Frank Zhu |
| **Maintainers** | Frank Zhu |
| **Last updated** | 2026-05-21 |
| **Supersedes** | v0.1, v0.2, v0.3, v0.4, v0.5, v0.6, v0.7, v0.8, v0.9, v0.10, v0.11, v0.12, v0.13, v0.14, v0.15, v0.16, v0.17, v0.18, v0.19, v0.20, v0.21, v0.22, v0.23, v0.24, v0.25, v0.26 |
| **Superseded by** | None |
| **Conflict rule** | This document supersedes earlier tzap format drafts. If it conflicts with v0.1-v0.26 text, this v0.27 draft wins unless a later dated spec explicitly supersedes it. |
| **File extension** | `.tzap` (single-volume) / `.tzap.NNN` (multi-volume) |

## Changelog from v0.26

This revision resolves implementation-readiness findings in v0.26:
missing magic-field validation requirements, a contradictory
CryptoHeader offset/gap rule, sparse local EnvelopeEntry table semantics,
underspecified path/hash cross-checks, and vague platform escape path
handling.

1. **Draft visibility is bumped.** `volume_format_rev = 27` for this
   draft. (§8, §17.1, §23, §28.1, §29)
2. **All magic fields are normative.** Writers must emit, and readers
   must validate, every fixed magic value before treating the containing
   structure as that type. (§8-§12.3, §15.9, §17.1, §28.1, §29)
3. **CryptoHeader placement is canonical.** Writers must set
   `crypto_header_offset = sizeof(VolumeHeader) = 128`; readers reject
   other offsets instead of tolerating unclaimed pre-CryptoHeader gaps.
   (§8, §17.1, §28.1, §29)
4. **Local EnvelopeEntry tables may be sparse.** Global payload envelope
   indices remain contiguous, but each IndexShard-local EnvelopeEntry
   table is only the sorted unique subset needed by that shard. (§15.3,
   §15.9, §28.1, §29)
5. **Stored path hashes are cross-checked.** Readers recompute
   FileEntry `path_hash` and DirectoryHintEntry `dir_hash` from the
   normalized string-pool bytes and reject mismatches. (§15.6, §15.8,
   §15.9, §28.1, §29)
6. **Platform escape paths are deterministic.** Archive paths and
   hardlink targets now have explicit host-independent rejection rules
   for backslash, drive/UNC/device namespaces, NTFS alternate streams,
   and Windows device names unless an explicit unsafe mode is requested.
   (§16, §28.1, §29)
...
a usable archive mode. Conformant v0.27 writers MUST set
`compression_algo = ZstdFramed`; Conformant v0.27 readers MUST reject
...
this draft, not usable archive modes. Conformant v0.27 writers MUST set
`fec_algo = ReedSolomonGF16`; Conformant v0.27 readers MUST reject any
...
- `pad_len = 0` is not valid in v0.27. The extra block in the exact-fit
...
stream), conforming v0.27 writers MUST use `stripe_width = 1`,
...
struct VolumeHeader {
    ...
    volume_format_rev:        u16,       // 27 for this draft
...
VolumeHeader and CryptoHeader. Writers MUST set `stripe_width ≥ 1`;
readers MUST reject a VolumeHeader whose `stripe_width = 0`.
...
parameters.
Readers MUST reject `compression_algo = None` in v0.27; payload and
...
known extensions are single-valued in v0.27; writers MUST NOT emit the
...
| ~~`0x0004`~~ | ~~`[u8; 32]`~~ | **Forbidden in v0.27.** The tar-stream content hash is encrypted inside IndexRoot. Writers MUST NOT emit this extension; readers MUST reject it if present. |
| `0x0005` | UTF-8 | Locale tag for filenames |
| ~~`0x0006`~~ | ~~bytes~~ | **Forbidden in v0.27; moved to encrypted metadata.** A writer setting `has_dictionary = 1` declares that IndexRoot locates a dictionary-object extent (§15.2). Writers MUST NOT emit this extension; readers MUST reject it if present. |
...
struct BlockRecord {
    ...
    flags:         u8,               // bit 0: last data block of encrypted object
                                     // bits 1..7: reserved; MUST be zero in v0.27
    _reserved:     [u8; 2],
...
MUST reject a BlockRecord with any reserved flag bit set; in v0.27 this
...
Completed v0.27 writers MUST set `is_authoritative = 1` in every closed
...
ManifestFooter and VolumeTrailer in v0.27.
...
struct BootstrapSidecarHeader {
    ...
    flags:                       u32,       // bit 0: ManifestFooter present
                                             ...
                                             // bits 3..31: reserved; MUST be zero in v0.27
...
be zero in v0.27; readers MUST reject the sidecar before trusting any
...
ManifestFooter HMAC and `archive_uuid`/`session_id`, MUST reject a v0.27
...
No padding, extension bytes, or unclaimed gaps are permitted in a v0.27
...
non-critical by a future version; for v0.27, unknown flag bits are a hard
error.
...
payload envelope sequence has no gaps. Because v0.27 stores only
...
For every encrypted object in v0.27, `encrypted_size` is the total
...
`FileEntry.flags` is reserved in v0.27. Writers MUST set it to zero, and
...
- when the same `frame_index` or `envelope_index` appears in more than
  ...
  version, this is equivalent to byte-identical row encoding in v0.27.
...
1. Read VolumeHeader at offset 0; verify CRC.
   ...
   by the reader. A reader claiming only v0.27 conformance rejects
   `volume_format_rev != 27`. Reject if `VolumeHeader.stripe_width = 0`
   or `crypto_header_offset != sizeof(VolumeHeader)`.
...
3. Reject unknown `compression_algo`, `aead_algo`, `fec_algo`, or
   ...
   reader-side cap in v0.27. Parse KdfParams only after `kdf_algo` is
...
In v0.27, archive parameters guarantee at least one payload parity
...
end. For a clean v0.27 core volume, after the reader has skipped any
...
be zero in v0.27 archives. Readers MUST reject blocks with reserved flag
bits set.
...
No part of the v0.27 write path needs to revisit a closed S3 part or to
...
writing final volumes. There is no conforming v0.27 mode that round-robins
...
### 20.1 Padding overhead (v0.27 unchanged from v0.15)
...
document uses `format_version = 1` and `volume_format_rev = 27`. Readers
...
`volume_format_rev = 27`; accepting earlier draft revisions requires an
...
implementation claims conformance to this v0.27 draft, incompatible
...
### 28.1 Test corpus additions for v0.27
...
- **CryptoHeader canonical offset**: set `crypto_header_offset` to values
  ...
  with padding bytes before the CryptoHeader; verify v0.27 readers reject
  instead of accepting an unclaimed gap.
- **Volume format revision**: create archives with `volume_format_rev`
  below, equal to, and above 27; verify v0.27-only readers accept only 27
...
- **CryptoHeader Extension TLVs**: with a valid HMAC, inject an unknown
  ...
  verify v0.27 readers ignore it and continue parsing. Repeat with the
...
- **Compression registry enforcement**: mutate CryptoHeader to
  `compression_algo = None`; verify v0.27 readers reject it before
...
1. Produces archives whose write sequence is strictly forward
   ...
   `format_version = 1`, `volume_format_rev = 27`, every fixed magic
...
23. Keeps global FrameEntry and EnvelopeEntry tables out of IndexRoot;
    each IndexShard carries the sorted local frame/envelope rows needed
    by its FileEntry records. IndexShard-local EnvelopeEntry tables may
    be sparse subsets of the global contiguous envelope sequence, but
    they contain no duplicate envelope indices and contain every envelope
    referenced by the shard's local FrameEntry rows.
...
25. Sets `FileEntry.flags = 0` and emits only NFC UTF-8 archive paths
    ...
    components, or other platform escape forms defined in §16). Computes
    every FileEntry `path_hash` and DirectoryHintEntry `dir_hash` from
    the exact normalized string-pool bytes stored for that row. Every
...
35. Emits v0.27 bootstrap sidecars as packed sequences with no padding,
...
1. On seekable input, locates the VolumeTrailer by seeking to
   ...
   whose `crypto_header_offset` is not `sizeof(VolumeHeader)`.
...
9. Validates IndexRoot, IndexShard, and DirectoryHintTable magic fields,
   structural counts, offsets, canonical table order, local sparse
   EnvelopeEntry semantics, stored path/hash bindings, and non-overlap
...
18. Rejects unknown algorithm IDs, `compression_algo != ZstdFramed`,
    ...
    caps, and rejects non-canonical CryptoHeader offsets. Before running
...
23. Rejects non-zero `FileEntry.flags`, unsafe archive paths, and
    ...
    rejecting unsafe hardlink targets, and rejecting or skipping symlinks
    that would escape the extraction root unless an explicit unsafe mode
    is requested.
...
## Appendix A: All changes from v0.26 → v0.27

| Section | Change |
|---|---|
| §8 / §17.1 / §23 / §28.1 / §29 | `volume_format_rev` is bumped to 27 and revision handling is aligned to v0.27 |
| §8-§12.3 / §15.9 / §17.1 / §28.1 / §29 | Fixed magic fields are explicit writer obligations and reader rejection checks for every structure type |
| §8 / §17.1 / §28.1 / §29 | `crypto_header_offset` is canonicalized to `sizeof(VolumeHeader) = 128`; unclaimed gaps before CryptoHeader are malformed |
| §15.3 / §15.9 / §28.1 / §29 | IndexShard-local EnvelopeEntry tables are defined as sorted unique sparse subsets, while full-archive verification still enforces contiguous global envelope indices |
| §15.6 / §15.8 / §15.9 / §28.1 / §29 | FileEntry and DirectoryHintEntry stored hash prefixes are recomputed from normalized string-pool bytes and mismatches are malformed |
| §16 / §28.1 / §29 | Platform escape paths are made deterministic, including backslash, colon/ADS, drive/UNC/device namespace forms, and Windows device-name components |

---

*End of v0.27 specification.*
