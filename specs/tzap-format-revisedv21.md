# tzap Archive Format Specification (v0.21)

| Field | Value |
|---|---|
| **Format version** | 1 |
| **Document version** | 0.21 / 2026-05-21.10 (self-reviewed draft after registry, trailer, and sidecar exactness cleanup) |
| **Status** | Draft for review, no implementation yet |
| **Owner** | Frank Zhu |
| **Maintainers** | Frank Zhu |
| **Last updated** | 2026-05-21 |
| **Supersedes** | v0.1, v0.2, v0.3, v0.4, v0.5, v0.6, v0.7, v0.8, v0.9, v0.10, v0.11, v0.12, v0.13, v0.14, v0.15, v0.16, v0.17, v0.18, v0.19, v0.20 |
| **Superseded by** | None |
| **Conflict rule** | This document supersedes earlier tzap format drafts. If it conflicts with v0.1-v0.20 text, this v0.21 draft wins unless a later dated spec explicitly supersedes it. |
| **File extension** | `.tzap` (single-volume) / `.tzap.NNN` (multi-volume) |

## Changelog from v0.20

This revision closes sequential-extraction and sidecar-HMAC ambiguities
found during external review.

1. **Draft visibility is bumped.** `volume_format_rev = 21` for this
   draft. (§8, §17.1, §23)
2. **Sidecar ManifestFooter HMAC is unambiguous.** A sidecar contains a
   freshly serialized sidecar ManifestFooter instance with
   `volume_index = 0`, `is_authoritative = 1`, and its own HMAC, not a
   byte-copy of a per-volume footer mutated after authentication. (§11,
   §12.2, §12.3, §19.1, §29)
3. **Upper-bound lookup boundary semantics are explicit.** If no entry's
   first hash is greater than the target hash, the landing candidate is
   the final table row; empty tables have no candidate. (§15.4, §15.7,
   §15.8)
4. **Sequential FEC repair has a parity-run rule.** Sequential readers
   infer payload-envelope parity from consecutive kind-1 blocks
   immediately following the envelope's data run, bounded by
   `fec_parity_shards`. (§17.3)
5. **Sequential clean close requires authentication.** A non-`TZBK`
   boundary or EOF is not proof of archive completion; readers must
   verify terminal authoritative ManifestFooter/VolumeTrailer
   authentication before reporting clean extraction or appending
   synthetic tar EOF blocks.
   (§17.3, §28.1, §29)
6. **Multi-volume non-seekable extraction is scoped.** The core format
   does not define a single-pipe multiplexing wrapper; striped
   non-seekable extraction requires concurrently supplied volume streams
   or an external wrapper that preserves volume/block identity. (§7.5,
   §17.3)
7. **Block final-flag validation is exact.** Each encrypted object's data
   run must contain exactly one last-data flag, on the final data block.
   (§10, §29)
8. **Size-product arithmetic is nailed down.** Readers must compute
   `data_block_count * block_size` with checked unsigned 64-bit-or-wider
   arithmetic before comparing to `encrypted_size`. (§15.9, §17.2, §29)
9. **DirectoryHintTable offset width is documented.** Its u64 offsets are
   wire-format fields for checked arithmetic/future room, but v0.21 still
   rejects values beyond the u32 object-size cage. (§15.8)
10. **Algorithm and trailer sanity checks are complete.** `fec_algo` is
    fixed to ReedSolomonGF16, CryptoHeader planning fields have explicit
    validity/advisory rules, `VolumeTrailer.block_count` is
    cross-checked against the BlockRecord region, the ManifestFooter must
    end exactly at the trailer offset, and sidecar copied
    BlockRecord sections must exactly match their object extents. (§5,
    §9, §12, §17.1, §28.1, §29)
...
a usable archive mode. Conformant v0.21 writers MUST set
`compression_algo = ZstdFramed`; conformant v0.21 readers MUST reject
...
this draft, not usable archive modes. Conformant v0.21 writers MUST set
`fec_algo = ReedSolomonGF16`; conformant v0.21 readers MUST reject any
...
- `pad_len = 0` is not valid in v0.21. The extra block in the exact-fit
...
stream), conforming v0.21 writers MUST use `stripe_width = 1`,
...
struct VolumeHeader {
    ...
    volume_format_rev:        u16,       // 21 for this draft
...
parameters.
Readers MUST reject `compression_algo = None` in v0.21; payload and
...
| ~~`0x0004`~~ | ~~`[u8; 32]`~~ | **Forbidden in v0.21.** The tar-stream content hash is encrypted inside IndexRoot. Writers MUST NOT emit this extension; readers MUST reject it if present. |
...
struct BlockRecord {
    ...
    flags:         u8,               // bit 0: last data block of encrypted object
                                     // bits 1..7: reserved; MUST be zero in v0.21
    _reserved:     [u8; 2],
...
MUST reject a BlockRecord with any reserved flag bit set; in v0.21 this
...
Completed v0.21 writers MUST set `is_authoritative = 1` in every closed
...
ManifestFooter and VolumeTrailer in v0.21.
...
struct BootstrapSidecarHeader {
    ...
    flags:                       u32,       // bit 0: ManifestFooter present
                                             ...
                                             // bits 3..31: reserved; MUST be zero in v0.21
...
ManifestFooter HMAC and `archive_uuid`/`session_id`, MUST reject a v0.21
...
No padding, extension bytes, or unclaimed gaps are permitted in a v0.21
...
non-critical by a future version; for v0.21, unknown flag bits are a hard
error.
...
For every encrypted object in v0.21, `encrypted_size` is the total
...
`FileEntry.flags` is reserved in v0.21. Writers MUST set it to zero, and
...
- when the same `frame_index` or `envelope_index` appears in more than
  ...
  version, this is equivalent to byte-identical row encoding in v0.21.
...
1. Read VolumeHeader at offset 0; verify CRC.
   ...
   only v0.21 conformance rejects `volume_format_rev != 21`.
...
3. Parse KdfParams; prompt for passphrase or load keyfile. If
   ...
   `m_cost_kib < 8 × parallelism`. Reject unknown algorithm IDs,
   `compression_algo != ZstdFramed`, `fec_algo != ReedSolomonGF16`,
   invalid boolean `has_dictionary`, `volume_loss_tolerance >=
   stripe_width`, `bit_rot_buffer_pct > 100`, or zero data-shard class
   maxima in v0.21.
...
6. If the input is seekable:
     ...
     d. Range-check `manifest_footer_offset` and
        ...
        exceed the selected trailer offset. Verify that
...
object. BlockRecords of kinds 2 through 9 may appear after payload
blocks in the same stream; a sequential extractor that is not
bootstrapping metadata MUST validate their BlockRecord framing/CRC and
skip them, not treat their presence as an envelope-counter event or as an
abort condition.
...
end. For a clean v0.21 core volume, after the reader has skipped any
...
be zero in v0.21 archives. Readers MUST reject blocks with reserved flag
bits set.
...
No part of the v0.21 write path needs to revisit a closed S3 part or to
...
writing final volumes. There is no conforming v0.21 mode that round-robins
...
### 20.1 Padding overhead (v0.21 unchanged from v0.15)
...
document uses `format_version = 1` and `volume_format_rev = 21`. Readers
...
`volume_format_rev = 21`; accepting earlier draft revisions requires an
...
implementation claims conformance to this v0.21 draft, incompatible
...
### 28.1 Test corpus additions for v0.21
...
- **Volume format revision**: create archives with `volume_format_rev`
  below, equal to, and above 21; verify v0.21-only readers accept only
  21 and reject newer revisions.
...
- **CryptoHeader parameter validity**: mutate `fec_algo` to `None` and
  ...
  `bit_rot_buffer_pct > 100`, and set each data-shard class maximum to
  zero or each FEC class-total maximum above configured reader caps;
  verify readers reject before object planning or FEC repair. Mutate
...
- **Compression registry enforcement**: mutate CryptoHeader to
  `compression_algo = None`; verify v0.21 readers reject it before
...
- **Bootstrap sidecar**: dictionary archive with a bootstrap sidecar;
  ...
  reject before trusting the sidecar object. Verify sidecars with
  padding, extension bytes, unclaimed gaps, or trailing bytes are
  rejected.
...
1. Produces archives whose write sequence is strictly forward
   ...
   `format_version = 1` and `volume_format_rev = 21`.
...
35. Emits v0.21 bootstrap sidecars as packed sequences with no padding,
...
18. Rejects unknown algorithm IDs, `compression_algo != ZstdFramed`,
    ...
    `chunk_size = 0`, and `chunk_size`, `envelope_target_size`, or
    `block_size` values above configured reader caps. It then treats the
    validated `chunk_size` and `expected_volume_size` as advisory
    metadata only; FrameEntry, EnvelopeEntry, and authenticated
    trailer/footer offsets remain authoritative.
...
23. Rejects non-zero `FileEntry.flags`, unsafe archive paths, and
    ...
    `data_block_count * block_size` computed with checked unsigned
...
## Appendix A: All changes from v0.20 → v0.21

| Section | Change |
|---|---|
| §11 / §12.2 / §12.3 / §19.1 / §28.1 / §29 | Sidecar ManifestFooter is freshly serialized with `volume_index = 0`, `is_authoritative = 1`, and its own HMAC; it is not a mutated byte-copy of a per-volume footer |
| §15.4 / §15.7 / §15.8 / §28.1 / §29 | Upper-bound lookup defines the no-greater-row case as landing on the final row |
| §17.3 / §28.1 / §29 | Sequential FEC repair infers payload-envelope parity from the consecutive kind-1 run after the last data block |
| §17.3 / §28.1 / §29 | Sequential extraction must authenticate authoritative terminal ManifestFooter/VolumeTrailer before reporting clean completion or appending synthetic tar EOF blocks |
| §7.5 / §17.3 / §28.1 | Non-seekable striped multi-volume extraction requires concurrent/framed volume streams; one raw concatenated pipe is not defined by the core format |
| §10 / §28.1 / §29 | Each encrypted object's data run must contain exactly one last-data flag on the final data block |
| §15.3 / §15.9 / §17.2 / §28.1 / §29 | `data_block_count * block_size` comparisons require checked unsigned 64-bit-or-wider arithmetic |
| §15.8 | DirectoryHintTable keeps u64 offsets but states they remain bounded by u32 decompressed size, plaintext length, and reader caps |
| §5 / §9 / §13.3 / §28.1 / §29 | `fec_algo` is fixed to ReedSolomonGF16; `has_dictionary`, volume-loss, bit-rot, and FEC class maxima have explicit reader rejection rules; `expected_volume_size` is advisory only |
| §12 / §17.1 / §17.3 / §28.1 / §29 | `VolumeTrailer.block_count` is cross-checked against the observed BlockRecord region before ManifestFooter, and ManifestFooter must end exactly at the trailer offset |
| §12.3 / §28.1 | Bootstrap sidecar BlockRecord sections must exactly match their IndexRoot/dictionary object extents with no missing, extra, duplicate, mis-sorted, or wrong-kind blocks |
| §28.1 / §29 | v0.21 tests and conformance checklist expanded |

---

*End of v0.21 specification.*
