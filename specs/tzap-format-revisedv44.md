# tzap Archive Format Specification (v0.44)

| Field | Value |
|---|---|
| **Format version** | 1 |
| **Document version** | 0.44 / 2026-06-17.1 (embedded recipient key wrapping) |
| **Status** | Draft implementation target |
| **Owner** | Frank Zhu |
| **Maintainers** | Frank Zhu |
| **Last updated** | 2026-06-17 |
| **Supersedes** | v0.1 … v0.43 |
| **Superseded by** | None |
| **Conflict rule** | This document is self-contained and supersedes all earlier tzap format drafts, including v0.43. If it conflicts with any v0.1–v0.43 text, this v0.44 draft wins unless a later dated spec explicitly supersedes it. |
| **File extension** | `.tzap` (single-volume) / `.volNNN.tzap` (multi-volume, e.g. `backup.vol000.tzap`, `backup.vol001.tzap`, …) |

## Changelog from v0.43

v0.44 keeps the v0.43 archive pipeline, optional plaintext mode, recovery
...
1. **Embedded recipient key wrapping.** A new `KdfAlgo::RecipientWrap` mode
   ...
   `CryptoHeader` (§9.3, §13.1). Each recipient record wraps the same 32-byte
   archive `master_key` to one recipient public encryption identity. A reader
   needs only one matching recipient private key to recover `master_key`, then
   follows the existing v1 symmetric subkey schedule.
2. **Public key-access metadata, authenticated after unwrap.** `KeyWrapTableV1`
   is visible without the archive key and is parsed only as untrusted bounded
   key-access material before `CryptoHeader` integrity succeeds. Its exact bytes
   are committed by a SHA-256 digest stored in `KdfParams`, then covered by
   `CryptoHeader.header_hmac` and RootAuth's `critical_metadata_digest`.
3. **Payload encryption remains unchanged.** Recipient wrapping changes only how
   `master_key` is obtained. AEAD object encryption, nonce/AAD construction,
   FEC, IndexRoot, and payload verification continue to use the v1 symmetric
   construction.
4. **Signing and recipient encryption remain separate.** RootAuth continues to
   answer "who signed this archive?" Recipient key wrapping answers "which
   private keys can recover this archive key?" The signing/X.509 authenticator
   plugins sign core's recomputed v0.44 `archive_root`; key-wrap profile logic
...
v44 terminates each closed volume with `ManifestFooter | RootAuthFooterV1? |
...
optional features. A conforming v0.44 archive may be any of these four
combinations:
...
a usable archive mode. Conformant v0.44 writers MUST set
`compression_algo = ZstdFramed`; Conformant v0.44 readers MUST reject
...
this draft, not usable archive modes. Conformant v0.44 writers MUST set
`fec_algo = ReedSolomonGF16`; Conformant v0.44 readers MUST reject any
...
`pad_len ≥ 255`. Wide form for `pad_len < 255` is non-canonical in v0.44 and
...
- `pad_len = 0` is not valid in v0.44. The extra block in the exact-fit
...
`CryptoHeader` and RootAuth critical metadata (§9.3, §30.9.1). v44 readers MUST
...
conforming v0.44 writers MUST use `stripe_width = 1`,
...
future volume data in memory. In v44, recipient-wrap streams carry
...
struct VolumeHeader {
    ...
    volume_format_rev:        u16,       // 44 for this draft
...
`header_digest = SHA-256(b"tzap-header-v44" || archive_uuid || session_id || all
...
`manifest_digest` (`b"tzap-manifest-v44"`, §11), `trailer_digest`
(`b"tzap-trailer-v44"`, §12), and `sidecar_digest` (`b"tzap-sidecar-v44"`,
§12.3). A v44 reader selects keyed-HMAC verification or unkeyed-digest
...
(§30). The distinct `tzap-*-v44` domains ensure an unkeyed digest can never be
...
parameters.
Readers MUST reject `compression_algo = None` in v0.44; payload and
...
known extensions are single-valued in v0.44; writers MUST NOT emit the
...
| ~~`0x0004`~~ | ~~`[u8; 32]`~~ | **Forbidden in v0.44.** The tar-stream content hash is encrypted inside IndexRoot. Writers MUST NOT emit this extension; readers MUST reject it if present. |
| `0x0005` | UTF-8 | Locale tag for filenames |
| ~~`0x0006`~~ | ~~bytes~~ | **Forbidden in v0.44; moved to encrypted metadata.** A writer setting `has_dictionary = 1` declares that IndexRoot locates a dictionary-object extent (§15.2). Writers MUST NOT emit this extension; readers MUST reject it if present. |
...
struct RecipientRecordV1 {
    ...
    flags:                      u32,     // reserved, MUST be zero in v44
...
struct BlockRecord {
    ...
    flags:         u8,               // bit 0: last data block of encrypted object
                                     // bits 1..7: reserved; MUST be zero in v0.44
    _reserved:     [u8; 2],
...
MUST reject a BlockRecord with any reserved flag bit set; in v0.44 this
...
`manifest_digest = SHA-256(b"tzap-manifest-v44" || archive_uuid || session_id ||
...
Completed v0.44 writers MUST set `is_authoritative = 1` in every closed
...
archive totals.
`index_root_first_block` has no "absent" sentinel in v0.44: IndexRoot is
...
Fixed 128 bytes. In v44 it is not the final bytes of the volume file. It holds
...
struct VolumeTrailer {
    ...
    volume_index:             u32,
    block_count:              u64,
    bytes_written:            u64,       // v44: absolute offset of this trailer
...
The v44 `VolumeTrailer` remains 128 bytes and keeps the v36 field order through
...
`trailer_digest = SHA-256(b"tzap-trailer-v44" || archive_uuid || session_id ||
first 96 bytes)` per §9, with the same coverage.

`block_count` is the number of BlockRecords physically written in this volume,
not the highest global `block_index`. For a completed conforming v44 volume, the
...
If root auth is enabled for a completed archive, every closed v44 volume MUST:
...
If root auth is absent, all four v44 root-auth trailer fields MUST be zero. Bits
1 through 31 of `root_auth_flags` are reserved and MUST be zero.

v44 redefines `VolumeTrailer.bytes_written`:
...
For a root-authenticated v44 volume:
...
of the needed blocks. v44 root-authenticated verification is available from a
...
only as a "valid v44 bootstrap sidecar" is a core bootstrap helper, not by
itself a core v44 root-auth authority.
...
struct BootstrapSidecarHeader {
    ...
    flags:                       u32,       // bit 0: ManifestFooter present
                                             ...
                                             // bits 3..31: reserved; MUST be zero in v0.44
...
be zero in v0.44; readers MUST reject the sidecar before trusting any
...
ManifestFooter integrity field and `archive_uuid`/`session_id`, MUST reject a v0.44
...
No padding, extension bytes, or unclaimed gaps are permitted in a v0.44
...
`sidecar_hmac` is instead the unkeyed `sidecar_digest = SHA-256(b"tzap-sidecar-v44"
...
non-critical by a future version; for v0.44, unknown flag bits are a hard
error.
...
`max_critical_recovery_scan` caps are part of the v44 critical-recovery cap
...
the v0.44 write-order invariant: a present dictionary object MUST NOT be
...
payload envelope sequence has no gaps. Because v0.44 stores only
...
For every encrypted object in v0.44, `encrypted_size` is the total
...
`FileEntry.flags` is reserved in v0.44. Writers MUST set it to zero, and
...
extension safety; a v0.44 DirectoryHintShardEntry in IndexRoot MUST NOT
...
- when the same `frame_index` or `envelope_index` appears in more than
  ...
  version, this is equivalent to byte-identical row encoding in v0.44.
...
2. If the input is seekable:
     ...
     c. Read the bounded CMRA byte range named by the accepted locator or scanned
        ...
        supplies the same tuple outside the on-disk v44 archive. Then validate
     ...
     f. Validate the recovered `VolumeHeader`: `magic = b"TZAP"`,
        `format_version = 1`, `volume_format_rev = 44`, valid
     ...
     h. Reject unknown `compression_algo`, `aead_algo`, `fec_algo`, or
        ...
        active reader-side cap in v0.44. Determine the protection mode from
...
3. If the input is non-seekable:
     a. Read `VolumeHeader` at offset 0; verify CRC. Reject if `magic != b"TZAP"`,
        `format_version != 1`, `volume_format_rev != 44`,
     ...
     c. If a trusted bootstrap sidecar is supplied, use it for ManifestFooter and
        ...
        itself a v44 root-auth authority unless an explicit external trusted API
...
In v0.44, archive parameters guarantee at least one payload parity
...
a clean v0.44 core volume, after the reader has skipped any post-payload
...
verified v44 terminal sequence to the reader.
...
Conforming v0.44 archives MUST NOT record a per-object
...
be zero in v0.44 archives. Readers MUST reject blocks with reserved flag
bits set.
...
16. For each sink, build the v44 VolumeTrailer with:
...
writing final volumes. There is no conforming v0.44 mode that round-robins
...
### 20.1 Padding overhead (v0.44 unchanged from v0.15)
...
- The registered AEADs are not specified as formally key-committing AEADs.
  ...
  signer-backed archive-level commitment use optional v44 root authentication;
...
document uses `format_version = 1` and `volume_format_rev = 44`. Readers MUST
...
draft MUST require `volume_format_rev = 44` for normal operation; accepting
earlier draft revisions requires an explicit compatibility mode. v44 readers
...
For a conforming v0.44 archive, every serialized class parity maximum
...
### 28.1 Test corpus additions through v0.44
...
High-priority v0.44 critical-recovery and root-auth regression cases:
...
15. Mutate v44 `VolumeTrailer.bytes_written` to physical EOF minus 128; readers
    reject because it does not equal the trailer offset.
16. Pipe a v44 single-volume archive through sequential extraction; output is
...
31. Pipe a non-seekable stream whose terminal tail exceeds
    ...
    output.
32. Mutate each new v44 structure's magic, version, format/revision field,
...
39. Pipe a root-authenticated v44 archive through a non-seekable sequential
...
58. Attempt non-seekable root-auth random access with a file described only as
    a "valid v44 bootstrap sidecar" and no external trusted API contract for its wire
    image, authority graph, and data-leaf commitments. Core v44 verification
    treats root auth as unavailable; the sidecar does not alter on-disk v44
...
Additional v0.44 structural regression cases:
...
Additional v0.44 boundary regression cases:
...
- **Unsafe parity conformance boundary**: attempt to emit a conforming archive
  ...
  v0.44 readers reject a purported conforming archive with mismatched class
...
- **Sequential provisional output**: stream a valid dictionary-free
  ...
  committed when v44 terminal verification fails after otherwise valid payload
  envelopes.
...
- **CryptoHeader canonical offset**: set `crypto_header_offset` to values
  ...
  with padding bytes before the CryptoHeader; verify v0.44 readers reject
...
- **CryptoHeader Extension TLVs**: with a valid mode-specific integrity field, inject an unknown
  ...
  verify v0.44 readers ignore it and continue parsing. Repeat with the
...
- **Compression registry enforcement**: mutate CryptoHeader to
  `compression_algo = None`; verify v0.44 readers reject it before
...
- **Sequential scan boundary**: append index, dictionary, and
  directory-hint BlockRecords after payload envelopes, then a valid v44 terminal
...
mode), a conformant v44 implementation MUST honor the protection mode (§5.1):
...
1. Produces archives whose write sequence is strictly forward
   ...
   `format_version = 1`, `volume_format_rev = 44`, every fixed magic
...
35. Emits v0.44 bootstrap sidecars as packed sequences with no padding,
...
Additional v44 writer requirements:

39. Sets `VolumeHeader.volume_format_rev = 44`, writes the v44 terminal sequence
    `ManifestFooter | RootAuthFooterV1? | VolumeTrailer | CMRA | LocatorMirror |
...
Additional v44 reader requirements:
...
32. Recovers and validates CMRA before using recovered critical metadata for
    ...
    windows.
33. Verifies v44 terminal adjacency and `VolumeTrailer.bytes_written` as the
    trailer offset, not physical EOF minus 128.
34. Performs full RootAuth verification only after the v44 full-archive
...
37. During full-archive verify or any operation that reports v44 content
...
This section is normative v44 core text. It integrates the root-auth v0.44
...
v44 keeps the payload pipeline defined in §6:
...
- Do not make v44 wire-compatible with v36.
- Do not require v36 readers to accept v44 archives.
...
### 30.2.2 Common v44 Rejection Rules

For every new v44 structure introduced by this v44 section, readers MUST reject
...
- every embedded `volume_format_rev` field equals `44`;
- every `_reserved`, reserved integer field, reserved byte range, and unknown
  flag bit is zero;
- every enum-like field is one of the values assigned by this v44 section, except
...
Unknown flag bits in v44 structures are not extension space for version 1
...
### 30.4 v44 VolumeTrailer Semantics

The v44 `VolumeTrailer` remains 128 bytes and keeps the existing field order.
The 20 reserved bytes are assigned in v44:
...
reserved and MUST be zero. If root auth is absent, all four v44 reserved fields
above MUST be zero.
...
It is the file size up to, but not including, `VolumeTrailer`. It is not
`physical_file_size - 128` in v44 because CMRA and two
...
struct CriticalMetadataImageV1 {
    ...
    manifest_footer_offset:        u64,
    manifest_footer_length:        u32,       // MUST be 136 for v44 base
...
`magic = b"TZMI"`, `version = 1`, `volume_format_rev = 44`,
...
mode-specific v44 terminal authority after `CryptoHeader`, `VolumeTrailer`, and
...
|---|---|---|
| `magic`, `version`, `volume_format_rev`, `layout_flags`, `_reserved`, `serialized_region_count`, every `SerializedRegion.region_type`, and every `SerializedRegion._reserved` | The fixed v44 structure rules in §30.2 and §30.5.1. Unknown flags, unknown regions, duplicate regions, non-canonical region order, and non-zero reserved bytes are hard rejection before any layout use. | Same fixed v44 structure rules. These are public corruption and syntax checks only. |
...
The final 256 bytes of every v44 volume are two locator copies:
...
`magic = b"TZCL"`, `version = 1`, `volume_format_rev = 44`,
...
`active_cmra_parity_pct_cap < 100` is a local resource policy, not a v44
...
image fields, four required region headers, the fixed v44 `VolumeHeader`,
...
than `max_critical_recovery_scan` bytes before EOF for v44 critical recovery.
...
emit at least this many CMRA parity shards for every closed v44 volume. A reader
that recovers an otherwise valid volume with fewer parity shards MUST reject it
as malformed before reporting the archive as a conforming v44 archive.
...
supplies the same decoder tuple outside the on-disk v44 archive. The parity
...
struct RootAuthFooterV1 {
    magic:                         [u8; 4],   // b"TZRA"
    version:                       u16,       // 1
    root_auth_spec_id:             [u8; 24],  // ASCII "tzap-root-auth-v0.44" + four zero bytes
...
- When root auth is enabled for a completed v44 archive, every closed volume
...
the 20 ASCII bytes `tzap-root-auth-v0.44` followed by four zero bytes,
`flags = 0`,
`format_version = 1`, `volume_format_rev = 44`, `_reserved` is all zero, the
...
bytes specified above. These checks are hard v44 wire acceptance rules before
...
All Merkle roots in this v44 section use the same tree algorithm.
...
v44 signs a canonical archive root, not raw volume bytes.
...
uses after its applicable v44 structure checks, CMRA recovery, source-authority
...
crypto_header_pre_hmac_digest = SHA-256(
    "tzap-crypto-header-pre-hmac-v44\0"
...
manifest_footer_global_pre_hmac_digest = SHA-256(
    "tzap-manifest-footer-global-pre-hmac-v44\0"
...
`index_digest = SHA-256("tzap-index-root-v44\0" || IndexRoot plaintext)`.
...
`tree_domain = "tzap-data-block-merkle-v44\0"`.
...
`root_auth_content_verified` is both a v44 full-archive content-conformance
result and a v44 root-auth result. The content-conformance gate is not a
literal legacy open/verify pass over v44 bytes. It imports the §15.9 and
full-archive tar/object checks after v44 terminal authorities have replaced the
old EOF and trailer-placement rules.

A reader MUST NOT report `root_auth_content_verified` until both conditions are
true:

1. The archive has passed the v44 full-archive content-conformance operation.
   This operation MUST:
   - bootstrap by §30.12, recover CMRA, parse the v44 terminal sequence, verify
   ...
   - apply the v44 terminal layout instead of the v36 physical EOF rule:
   ...
   - run the §15.9 structural validation and full-archive checks for exact
     ...
     v44 mode-specifically verified `CryptoHeader`, `ManifestFooter`,
...
would otherwise pass but the v44 full-archive content-conformance operation
fails or has not run, the reader MUST NOT report
`root_auth_content_verified`.

After CMRA repair, successful v44 full-archive content conformance, and the
...
Root-authenticated v44 verification MUST NOT require bytes from a wholly lost
...
- `root_auth_content_verified`: the authenticator verified `archive_root`,
  ...
  data BlockRecord payloads, and the archive also passed the v44 full-archive
...
1. `VolumeHeader` MUST be exactly the v44 fixed header at offset 0:
   `magic = b"TZAP"`, `format_version = 1`, `volume_format_rev = 44`,
...
3. `compression_algo`, `aead_algo`, `fec_algo`, and `kdf_algo` MUST be values
   defined by §5. In v44 public no-key mode, `compression_algo` MUST be
   `ZstdFramed` and `fec_algo` MUST be `ReedSolomonGF16`, matching the v44
...
4. `CryptoHeader` reserved bytes MUST be zero. The Extension TLV list MUST
   ...
   described as successful v44 public verification. Unknown non-critical
...
3. Every public `VolumeHeader` agrees with `RootAuthFooterV1` on
   ...
   `volume_format_rev = 44`. The accepted recovered image identity fields and
...
3. Validate at least one CMRA decoder envelope: the CMRA header, a locator
   ...
   the same tuple outside the on-disk v44 archive. Then validate shard CRCs.
...
9. If root auth is present and requested, run the v44 full-archive
   ...
   the operation has not retained or scanned enough state for either the v44
...
### 30.13 Non-seekable and Sequential v44

v44 keeps the provisional-output rule defined in §17.3.
...
part of the on-disk v44 archive format and does not change the v44 terminal
...
The §12.3 v44 bootstrap sidecar remains a core bootstrap helper for
...
serialization. Core v44 defines no additional root-auth sidecar-v2 wire image,
...
1. Choose v44 options, all FEC class maxima, and a default-interoperable CMRA
...
7. Build `VolumeTrailer` with v44 root-auth pointer fields and compute its
   mode-specific integrity field.
...
## Appendix A: Major changes from v0.43 to v0.44

v0.44 builds on v0.43 and adds embedded recipient key wrapping while preserving
the v1 symmetric encryption construction.
