# tzap Archive Format Specification (v0.28)

| Field | Value |
|---|---|
| **Format version** | 1 |
| **Document version** | 0.28 / 2026-05-21.4 (implementation-readiness fixes) |
| **Status** | Draft for review, no implementation yet |
| **Owner** | Frank Zhu |
| **Maintainers** | Frank Zhu |
| **Last updated** | 2026-05-21 |
| **Supersedes** | v0.1, v0.2, v0.3, v0.4, v0.5, v0.6, v0.7, v0.8, v0.9, v0.10, v0.11, v0.12, v0.13, v0.14, v0.15, v0.16, v0.17, v0.18, v0.19, v0.20, v0.21, v0.22, v0.23, v0.24, v0.25, v0.26, v0.27 |
| **Superseded by** | None |
| **Conflict rule** | This document supersedes earlier tzap format drafts. If it conflicts with v0.1-v0.27 text, this v0.28 draft wins unless a later dated spec explicitly supersedes it. |
| **File extension** | `.tzap` (single-volume) / `.tzap.NNN` (multi-volume) |

## Changelog from v0.27

This revision resolves implementation-readiness findings in v0.27:
missing volume-index bounds, ambiguous duplicate supplied-volume
handling, underspecified shard-local FrameEntry table exactness,
non-canonical root directory-hint path offsets, hardlink target
resolution safety gaps, missing cross-object block-extent overlap
checks, and a stale CryptoHeader TLV test length.

1. **Draft visibility is bumped.** `volume_format_rev = 28` for this
   draft. (§8, §17.1, §23, §28.1, §29)
2. **Volume indexes are bounded.** Writers must emit volume indexes in
   `[0, stripe_width)`. Readers reject out-of-range authenticated volume
   indexes and duplicate supplied volume indexes unless an explicit
   duplicate-copy recovery mode proves byte-for-byte identity. (§8, §10,
   §17.1, §28.1, §29)
3. **Shard-local frame tables are exact.** Each IndexShard-local
   FrameEntry table is the sorted unique exact set required by that
   shard's FileEntry ranges; each local EnvelopeEntry table is the sorted
   unique exact set required by those frames. (§15.3, §15.9, §29)
4. **Directory-hint root encoding is canonical.** A zero-length
   DirectoryHintEntry path is only the root directory, must use
   `path_offset = 0`, and must hash `SHA-256(b"")[0..8]`; non-root hint
   paths must have an in-bounds string-pool range. (§15.8, §15.9,
   §28.1, §29)
5. **Hardlink target resolution is no-follow and deterministic.**
   Default readers may create hardlinks only to already-restored safe
   non-directory regular-file targets resolved without following
   symlinks or reparse/alias objects. (§16, §28.1, §29)
6. **Object block extents cannot overlap.** Writers must assign
   non-overlapping global block-index ranges to encrypted objects, and
   readers/full verification reject inconsistent overlaps before object
   use. (§10, §15.9, §18, §28.1, §29)
7. **The TLV cap test reference is corrected.** The CryptoHeader
   Extension TLV test corpus now uses the normative 256-byte payload cap,
   not a stale 266-byte threshold. (§28.1)
...
a usable archive mode. Conformant v0.28 writers MUST set
`compression_algo = ZstdFramed`; Conformant v0.28 readers MUST reject
...
this draft, not usable archive modes. Conformant v0.28 writers MUST set
`fec_algo = ReedSolomonGF16`; Conformant v0.28 readers MUST reject any
...
- `pad_len = 0` is not valid in v0.28. The extra block in the exact-fit
...
stream), conforming v0.28 writers MUST use `stripe_width = 1`,
...
struct VolumeHeader {
    ...
    volume_format_rev:        u16,       // 28 for this draft
...
parameters.
Readers MUST reject `compression_algo = None` in v0.28; payload and
...
known extensions are single-valued in v0.28; writers MUST NOT emit the
...
| ~~`0x0004`~~ | ~~`[u8; 32]`~~ | **Forbidden in v0.28.** The tar-stream content hash is encrypted inside IndexRoot. Writers MUST NOT emit this extension; readers MUST reject it if present. |
| `0x0005` | UTF-8 | Locale tag for filenames |
| ~~`0x0006`~~ | ~~bytes~~ | **Forbidden in v0.28; moved to encrypted metadata.** A writer setting `has_dictionary = 1` declares that IndexRoot locates a dictionary-object extent (§15.2). Writers MUST NOT emit this extension; readers MUST reject it if present. |
...
struct BlockRecord {
    ...
    flags:         u8,               // bit 0: last data block of encrypted object
                                     // bits 1..7: reserved; MUST be zero in v0.28
    _reserved:     [u8; 2],
...
MUST reject a BlockRecord with any reserved flag bit set; in v0.28 this
...
Completed v0.28 writers MUST set `is_authoritative = 1` in every closed
...
ManifestFooter and VolumeTrailer in v0.28.
...
struct BootstrapSidecarHeader {
    ...
    flags:                       u32,       // bit 0: ManifestFooter present
                                             ...
                                             // bits 3..31: reserved; MUST be zero in v0.28
...
be zero in v0.28; readers MUST reject the sidecar before trusting any
...
ManifestFooter HMAC and `archive_uuid`/`session_id`, MUST reject a v0.28
...
No padding, extension bytes, or unclaimed gaps are permitted in a v0.28
...
non-critical by a future version; for v0.28, unknown flag bits are a hard
error.
...
payload envelope sequence has no gaps. Because v0.28 stores only
...
For every encrypted object in v0.28, `encrypted_size` is the total
...
`FileEntry.flags` is reserved in v0.28. Writers MUST set it to zero, and
...
- when the same `frame_index` or `envelope_index` appears in more than
  ...
  version, this is equivalent to byte-identical row encoding in v0.28.
...
1. Read VolumeHeader at offset 0; verify CRC.
   ...
   by the reader. A reader claiming only v0.28 conformance rejects
   `volume_format_rev != 28`. Reject if `VolumeHeader.stripe_width = 0`
...
3. Reject unknown `compression_algo`, `aead_algo`, `fec_algo`, or
   ...
   reader-side cap in v0.28. Parse KdfParams only after `kdf_algo` is
...
In v0.28, archive parameters guarantee at least one payload parity
...
end. For a clean v0.28 core volume, after the reader has skipped any
...
be zero in v0.28 archives. Readers MUST reject blocks with reserved flag
bits set.
...
No part of the v0.28 write path needs to revisit a closed S3 part or to
...
writing final volumes. There is no conforming v0.28 mode that round-robins
...
### 20.1 Padding overhead (v0.28 unchanged from v0.15)
...
document uses `format_version = 1` and `volume_format_rev = 28`. Readers
...
`volume_format_rev = 28`; accepting earlier draft revisions requires an
...
implementation claims conformance to this v0.28 draft, incompatible
...
### 28.1 Test corpus additions for v0.28
...
- **CryptoHeader canonical offset**: set `crypto_header_offset` to values
  ...
  with padding bytes before the CryptoHeader; verify v0.28 readers reject
  instead of accepting an unclaimed gap.
- **Volume format revision**: create archives with `volume_format_rev`
  below, equal to, and above 28; verify v0.28-only readers accept only 28
...
- **CryptoHeader Extension TLVs**: with a valid HMAC, inject an unknown
  ...
  verify v0.28 readers ignore it and continue parsing. Repeat with the
...
- **Compression registry enforcement**: mutate CryptoHeader to
  `compression_algo = None`; verify v0.28 readers reject it before
...
1. Produces archives whose write sequence is strictly forward
   ...
   `format_version = 1`, `volume_format_rev = 28`, every fixed magic
...
35. Emits v0.28 bootstrap sidecars as packed sequences with no padding,
...
9. Validates IndexRoot, IndexShard, and DirectoryHintTable magic fields,
   ...
   bindings, and non-overlap before allocation or indexing.
...
## Appendix A: All changes from v0.27 → v0.28

| Section | Change |
|---|---|
| §8 / §10 / §17.1 / §28.1 / §29 | `volume_format_rev` is bumped to 28 and volume indexes are bounded to `[0, stripe_width)`, with duplicate supplied volume indexes rejected by default |
| §15.3 / §15.9 / §29 | IndexShard-local FrameEntry tables are exact sorted unique subsets required by FileEntry ranges, and local EnvelopeEntry tables are the exact sorted unique envelope set referenced by those frames |
| §15.8 / §15.9 / §28.1 / §29 | DirectoryHintEntry root paths are canonicalized to `path_length = 0`, `path_offset = 0`, and `SHA-256(b"")[0..8]`; non-root hint path ranges must be in-bounds |
| §16 / §28.1 / §29 | Hardlink target handling now requires no-follow target validation and default hardlinks only to already-restored safe regular-file targets |
| §10 / §15.9 / §18 / §28.1 / §29 | Distinct encrypted-object block ranges must not overlap, and full-archive verification checks overlap globally |
| §28.1 | CryptoHeader Extension TLV malformed-length corpus entry uses the normative 256-byte payload cap |

---

*End of v0.28 specification.*
