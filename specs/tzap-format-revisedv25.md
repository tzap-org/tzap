# tzap Archive Format Specification (v0.25)

| Field | Value |
|---|---|
| **Format version** | 1 |
| **Document version** | 0.25 / 2026-05-21.1 (authenticated extension parsing fix) |
| **Status** | Draft for review, no implementation yet |
| **Owner** | Frank Zhu |
| **Maintainers** | Frank Zhu |
| **Last updated** | 2026-05-21 |
| **Supersedes** | v0.1, v0.2, v0.3, v0.4, v0.5, v0.6, v0.7, v0.8, v0.9, v0.10, v0.11, v0.12, v0.13, v0.14, v0.15, v0.16, v0.17, v0.18, v0.19, v0.20, v0.21, v0.22, v0.23, v0.24 |
| **Superseded by** | None |
| **Conflict rule** | This document supersedes earlier tzap format drafts. If it conflicts with v0.1-v0.24 text, this v0.25 draft wins unless a later dated spec explicitly supersedes it. |
| **File extension** | `.tzap` (single-volume) / `.tzap.NNN` (multi-volume) |

## Changelog from v0.24

This revision resolves implementation-readiness findings in v0.24:
CryptoHeader extension TLV framing, authenticated extension semantics,
and stale revision metadata.

1. **Draft visibility is bumped.** `volume_format_rev = 25` for this
   draft. (§8, §17.1, §23)
2. **Extension framing is strict before HMAC.** Readers may structurally
   scan untrusted Extension TLVs only to prove bounds, length caps,
   terminator placement, and absence of trailing bytes before HMAC.
   (§9, §17.1, §28.1, §29)
3. **Extension semantics are authenticated.** Readers interpret known,
   forbidden, duplicate, unknown-critical, and unknown-non-critical
   extensions only after CryptoHeader HMAC verification succeeds.
   (§9, §17.1, §29)
4. **Removed dictionary-hash extension is forbidden.** Extension `0x0006`
   is explicitly rejected if present, matching its migration into
   encrypted IndexRoot metadata. (§9.1, §28.1, §29)
...
a usable archive mode. Conformant v0.25 writers MUST set
`compression_algo = ZstdFramed`; Conformant v0.25 readers MUST reject
...
this draft, not usable archive modes. Conformant v0.25 writers MUST set
`fec_algo = ReedSolomonGF16`; Conformant v0.25 readers MUST reject any
...
- `pad_len = 0` is not valid in v0.25. The extra block in the exact-fit
...
stream), conforming v0.25 writers MUST use `stripe_width = 1`,
...
struct VolumeHeader {
    ...
    volume_format_rev:        u16,       // 25 for this draft
...
parameters.
Readers MUST reject `compression_algo = None` in v0.25; payload and
...
known extensions are single-valued in v0.25; writers MUST NOT emit the
...
| ~~`0x0004`~~ | ~~`[u8; 32]`~~ | **Forbidden in v0.25.** The tar-stream content hash is encrypted inside IndexRoot. Writers MUST NOT emit this extension; readers MUST reject it if present. |
| `0x0005` | UTF-8 | Locale tag for filenames |
| ~~`0x0006`~~ | ~~bytes~~ | **Forbidden in v0.25; moved to encrypted metadata.** A writer setting `has_dictionary = 1` declares that IndexRoot locates a dictionary-object extent (§15.2). Writers MUST NOT emit this extension; readers MUST reject it if present. |
...
struct BlockRecord {
    ...
    flags:         u8,               // bit 0: last data block of encrypted object
                                     // bits 1..7: reserved; MUST be zero in v0.25
    _reserved:     [u8; 2],
...
MUST reject a BlockRecord with any reserved flag bit set; in v0.25 this
...
Completed v0.25 writers MUST set `is_authoritative = 1` in every closed
...
ManifestFooter and VolumeTrailer in v0.25.
...
struct BootstrapSidecarHeader {
    ...
    flags:                       u32,       // bit 0: ManifestFooter present
                                             ...
                                             // bits 3..31: reserved; MUST be zero in v0.25
...
be zero in v0.25; readers MUST reject the sidecar before trusting any
...
ManifestFooter HMAC and `archive_uuid`/`session_id`, MUST reject a v0.25
...
No padding, extension bytes, or unclaimed gaps are permitted in a v0.25
...
non-critical by a future version; for v0.25, unknown flag bits are a hard
error.
...
For every encrypted object in v0.25, `encrypted_size` is the total
...
`FileEntry.flags` is reserved in v0.25. Writers MUST set it to zero, and
...
- when the same `frame_index` or `envelope_index` appears in more than
  ...
  version, this is equivalent to byte-identical row encoding in v0.25.
...
1. Read VolumeHeader at offset 0; verify CRC.
   ...
   only v0.25 conformance rejects `volume_format_rev != 25`.
...
3. Parse KdfParams; prompt for passphrase or load keyfile. If
   ...
   terminator and `header_hmac`. Reject unknown algorithm IDs,
...
In v0.25, archive parameters guarantee at least one payload parity
...
end. For a clean v0.25 core volume, after the reader has skipped any
...
be zero in v0.25 archives. Readers MUST reject blocks with reserved flag
bits set.
...
No part of the v0.25 write path needs to revisit a closed S3 part or to
...
writing final volumes. There is no conforming v0.25 mode that round-robins
...
### 20.1 Padding overhead (v0.25 unchanged from v0.15)
...
document uses `format_version = 1` and `volume_format_rev = 25`. Readers
...
`volume_format_rev = 25`; accepting earlier draft revisions requires an
...
implementation claims conformance to this v0.25 draft, incompatible
...
### 28.1 Test corpus additions for v0.25
...
- **Volume format revision**: create archives with `volume_format_rev`
  below, equal to, and above 25; verify v0.25-only readers accept only 25 and reject older or newer revisions.
...
- **CryptoHeader Extension TLVs**: with a valid HMAC, inject an unknown
  ...
  verify v0.25 readers ignore it and continue parsing. Repeat with the
...
- **Compression registry enforcement**: mutate CryptoHeader to
  `compression_algo = None`; verify v0.25 readers reject it before
...
1. Produces archives whose write sequence is strictly forward
   ...
   `format_version = 1` and `volume_format_rev = 25`.
...
35. Emits v0.25 bootstrap sidecars as packed sequences with no padding,
...
18. Rejects unknown algorithm IDs, `compression_algo != ZstdFramed`,
    ...
    `block_size` values above configured reader caps. Before running the
    KDF, verifies that the selected KdfParams payload fits inside
    CryptoHeader and structurally scans Extension TLVs only for bounded
    framing, payload length caps, valid terminator encoding, and exact
    terminator placement before the HMAC. It then
...
## Appendix A: All changes from v0.24 → v0.25

| Section | Change |
|---|---|
| §8 / §17.1 / §23 / §28.1 / §29 | `volume_format_rev` is bumped to 25 and revision handling is aligned to v0.25 |
| §9 / §17.1 / §29 | CryptoHeader Extension TLV framing is separated from authenticated extension semantics |
| §9 / §28.1 / §29 | Terminator encoding, terminator placement, extension length caps, duplicate known tags, and unknown critical extensions are explicitly validated |
| §9.1 / §28.1 / §29 | Removed extension tag `0x0006` is now explicitly forbidden and rejected if present |
| §28.1 / §29 | v0.25 tests and conformance checklist expanded for authenticated extension parsing |

---

*End of v0.25 specification.*
