# tzap Archive Format Specification (v0.30)

| Field | Value |
|---|---|
| **Format version** | 1 |
| **Document version** | 0.30 / 2026-05-21.6 (implementation-readiness fixes) |
| **Status** | Draft for review, no implementation yet |
| **Owner** | Frank Zhu |
| **Maintainers** | Frank Zhu |
| **Last updated** | 2026-05-21 |
| **Supersedes** | v0.1, v0.2, v0.3, v0.4, v0.5, v0.6, v0.7, v0.8, v0.9, v0.10, v0.11, v0.12, v0.13, v0.14, v0.15, v0.16, v0.17, v0.18, v0.19, v0.20, v0.21, v0.22, v0.23, v0.24, v0.25, v0.26, v0.27, v0.28, v0.29 |
| **Superseded by** | None |
| **Conflict rule** | This document supersedes earlier tzap format drafts. If it conflicts with v0.1-v0.29 text, this v0.30 draft wins unless a later dated spec explicitly supersedes it. |
| **File extension** | `.tzap` (single-volume) / `.tzap.NNN` (multi-volume) |

## Changelog from v0.29

This revision resolves implementation-readiness findings in v0.29:
underspecified bootstrap-sidecar sparse-section authority, ambiguous
Argon2id passphrase byte canonicalization, ambiguous handling of global
tar metadata records, and an imprecise directory-hint shard-count cap
reference.

1. **Draft visibility is bumped.** `volume_format_rev = 30` for this
   draft. (§8, §17.1, §23, §28.1, §29)
2. **Bootstrap sidecar sparse sections have explicit authorities.**
   Sidecar BlockRecord sections are usable only when the reader has an
   authenticated ManifestFooter and, for dictionary records, an
   authenticated IndexRoot from the sidecar or another source. A
   non-seekable sidecar bootstrap must carry the ManifestFooter and
   IndexRoot records. (§12.3, §17.1, §28.1, §29)
3. **Argon2id passphrase bytes are canonicalized.** The KDF input is the
   exact UTF-8 encoding of the caller-supplied Unicode passphrase after
   NFC normalization, with no implicit newline, BOM, NUL, locale, or
   platform-charset transformation by the archive format. (§13.1, §28.1,
   §29)
4. **Tar global metadata records are forbidden in stored member groups.**
   tzap tar member groups may contain only path-specific metadata records
   needed for one main entry; global PAX or other non-path-specific tar
   state is rejected to keep FileEntry binding deterministic. (§16,
   §28.1, §29)
5. **Directory-hint shard-count caps are named precisely.** Structural
   validation now references `max_directory_hint_shards`, not only the
   generic shard cap. (§15.9, §28.1, §29)
...
a usable archive mode. Conformant v0.30 writers MUST set
`compression_algo = ZstdFramed`; Conformant v0.30 readers MUST reject
...
this draft, not usable archive modes. Conformant v0.30 writers MUST set
`fec_algo = ReedSolomonGF16`; Conformant v0.30 readers MUST reject any
...
- `pad_len = 0` is not valid in v0.30. The extra block in the exact-fit
...
stream), conforming v0.30 writers MUST use `stripe_width = 1`,
...
struct VolumeHeader {
    ...
    volume_format_rev:        u16,       // 30 for this draft
...
parameters.
Readers MUST reject `compression_algo = None` in v0.30; payload and
...
known extensions are single-valued in v0.30; writers MUST NOT emit the
...
| ~~`0x0004`~~ | ~~`[u8; 32]`~~ | **Forbidden in v0.30.** The tar-stream content hash is encrypted inside IndexRoot. Writers MUST NOT emit this extension; readers MUST reject it if present. |
| `0x0005` | UTF-8 | Locale tag for filenames |
| ~~`0x0006`~~ | ~~bytes~~ | **Forbidden in v0.30; moved to encrypted metadata.** A writer setting `has_dictionary = 1` declares that IndexRoot locates a dictionary-object extent (§15.2). Writers MUST NOT emit this extension; readers MUST reject it if present. |
...
struct BlockRecord {
    ...
    flags:         u8,               // bit 0: last data block of encrypted object
                                     // bits 1..7: reserved; MUST be zero in v0.30
    _reserved:     [u8; 2],
...
MUST reject a BlockRecord with any reserved flag bit set; in v0.30 this
...
Completed v0.30 writers MUST set `is_authoritative = 1` in every closed
...
ManifestFooter and VolumeTrailer in v0.30.
...
struct BootstrapSidecarHeader {
    ...
    flags:                       u32,       // bit 0: ManifestFooter present
                                             ...
                                             // bits 3..31: reserved; MUST be zero in v0.30
...
be zero in v0.30; readers MUST reject the sidecar before trusting any
...
ManifestFooter HMAC and `archive_uuid`/`session_id`, MUST reject a v0.30
...
No padding, extension bytes, or unclaimed gaps are permitted in a v0.30
...
non-critical by a future version; for v0.30, unknown flag bits are a hard
error.
...
payload envelope sequence has no gaps. Because v0.30 stores only
...
For every encrypted object in v0.30, `encrypted_size` is the total
...
`FileEntry.flags` is reserved in v0.30. Writers MUST set it to zero, and
...
- when the same `frame_index` or `envelope_index` appears in more than
  ...
  version, this is equivalent to byte-identical row encoding in v0.30.
...
1. Read VolumeHeader at offset 0; verify CRC.
   ...
   by the reader. A reader claiming only v0.30 conformance rejects
   `volume_format_rev != 30`. Reject if `VolumeHeader.stripe_width = 0`
...
3. Reject unknown `compression_algo`, `aead_algo`, `fec_algo`, or
   ...
   reader-side cap in v0.30. Parse KdfParams only after `kdf_algo` is
...
7. If the input is non-seekable:
     a. If a trusted bootstrap sidecar is supplied, use it for
        ...
        64-bit arithmetic or wider before using the IndexRoot extent.
...
In v0.30, archive parameters guarantee at least one payload parity
...
end. For a clean v0.30 core volume, after the reader has skipped any
...
be zero in v0.30 archives. Readers MUST reject blocks with reserved flag
bits set.
...
No part of the v0.30 write path needs to revisit a closed S3 part or to
...
writing final volumes. There is no conforming v0.30 mode that round-robins
...
### 20.1 Padding overhead (v0.30 unchanged from v0.15)
...
document uses `format_version = 1` and `volume_format_rev = 30`. Readers
...
`volume_format_rev = 30`; accepting earlier draft revisions requires an
...
implementation claims conformance to this v0.30 draft, incompatible
...
### 28.1 Test corpus additions for v0.30
...
- **CryptoHeader canonical offset**: set `crypto_header_offset` to values
  ...
  with padding bytes before the CryptoHeader; verify v0.30 readers reject
  instead of accepting an unclaimed gap.
- **Volume format revision**: create archives with `volume_format_rev`
  below, equal to, and above 29; verify v0.30-only readers accept only 30
...
- **CryptoHeader Extension TLVs**: with a valid HMAC, inject an unknown
  ...
  verify v0.30 readers ignore it and continue parsing. Repeat with the
...
- **Compression registry enforcement**: mutate CryptoHeader to
  `compression_algo = None`; verify v0.30 readers reject it before
...
1. Produces archives whose write sequence is strictly forward
   ...
   `format_version = 1`, `volume_format_rev = 30`, every fixed magic
...
35. Emits v0.30 bootstrap sidecars as packed sequences with no padding,
...
## Appendix A: All changes from v0.29 -> v0.30

| Section | Change |
|---|---|
| §8 / §17.1 / §23 / §28.1 / §29 | `volume_format_rev` is bumped to 30 |
| §12.3 / §17.1 / §28.1 / §29 | Defines authority requirements for sparse bootstrap sidecar sections and requires ManifestFooter plus IndexRoot sections for non-seekable sidecar bootstrap |
| §13.1 / §28.1 / §29 | Defines the exact Argon2id passphrase byte string as UTF-8 of the NFC-normalized caller passphrase, with no implicit archive-format trimming or transcoding |
| §16 / §28.1 / §29 | Forbids global PAX/GNU or other non-path-specific tar metadata records in the encrypted tzap tar stream |
| §15.9 / §28.1 / §29 | Names `max_directory_hint_shards` as the cap for `directory_hint_shard_count` |

---

*End of v0.30 specification.*
