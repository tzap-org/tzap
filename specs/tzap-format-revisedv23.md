# tzap Archive Format Specification (v0.23)

| Field | Value |
|---|---|
| **Format version** | 1 |
| **Document version** | 0.23 / 2026-05-21.3 (self-reviewed implementation-readiness draft) |
| **Status** | Draft for review, no implementation yet |
| **Owner** | Frank Zhu |
| **Maintainers** | Frank Zhu |
| **Last updated** | 2026-05-21 |
| **Supersedes** | v0.1, v0.2, v0.3, v0.4, v0.5, v0.6, v0.7, v0.8, v0.9, v0.10, v0.11, v0.12, v0.13, v0.14, v0.15, v0.16, v0.17, v0.18, v0.19, v0.20, v0.21, v0.22 |
| **Superseded by** | None |
| **Conflict rule** | This document supersedes earlier tzap format drafts. If it conflicts with v0.1-v0.22 text, this v0.23 draft wins unless a later dated spec explicitly supersedes it. |
| **File extension** | `.tzap` (single-volume) / `.tzap.NNN` (multi-volume) |

## Changelog from v0.22

This revision resolves final implementation-readiness findings in v0.22:
one stale revision test and four areas where conforming readers could
still make different operational choices.

1. **Draft visibility is bumped.** `volume_format_rev = 23` for this
   draft. (§8, §17.1, §23)
...
a usable archive mode. Conformant v0.23 writers MUST set
`compression_algo = ZstdFramed`; conformant v0.23 readers MUST reject
...
this draft, not usable archive modes. Conformant v0.23 writers MUST set
`fec_algo = ReedSolomonGF16`; conformant v0.23 readers MUST reject any
...
- `pad_len = 0` is not valid in v0.23. The extra block in the exact-fit
...
stream), conforming v0.23 writers MUST use `stripe_width = 1`,
...
struct VolumeHeader {
    ...
    volume_format_rev:        u16,       // 23 for this draft
...
parse Extension TLVs. Readers MUST reject a CryptoHeader whose TLV list
...
parameters.
Readers MUST reject `compression_algo = None` in v0.23; payload and
...
| ~~`0x0004`~~ | ~~`[u8; 32]`~~ | **Forbidden in v0.23.** The tar-stream content hash is encrypted inside IndexRoot. Writers MUST NOT emit this extension; readers MUST reject it if present. |
...
struct BlockRecord {
    ...
    flags:         u8,               // bit 0: last data block of encrypted object
                                     // bits 1..7: reserved; MUST be zero in v0.23
    _reserved:     [u8; 2],
...
MUST reject a BlockRecord with any reserved flag bit set; in v0.23 this
...
Completed v0.23 writers MUST set `is_authoritative = 1` in every closed
...
ManifestFooter and VolumeTrailer in v0.23.
...
struct BootstrapSidecarHeader {
    ...
    flags:                       u32,       // bit 0: ManifestFooter present
                                             ...
                                             // bits 3..31: reserved; MUST be zero in v0.23
...
be zero in v0.23; readers MUST reject the sidecar before trusting any
...
ManifestFooter HMAC and `archive_uuid`/`session_id`, MUST reject a v0.23
...
No padding, extension bytes, or unclaimed gaps are permitted in a v0.23
...
non-critical by a future version; for v0.23, unknown flag bits are a hard
error.
...
For every encrypted object in v0.23, `encrypted_size` is the total
...
`FileEntry.flags` is reserved in v0.23. Writers MUST set it to zero, and
...
- when the same `frame_index` or `envelope_index` appears in more than
  ...
  version, this is equivalent to byte-identical row encoding in v0.23.
...
1. Read VolumeHeader at offset 0; verify CRC.
   ...
   only v0.23 conformance rejects `volume_format_rev != 23`.
...
3. Parse KdfParams; prompt for passphrase or load keyfile. If
   ...
   `bit_rot_buffer_pct > 100`, or zero data-shard class maxima in v0.23.
...
In v0.23, archive parameters guarantee at least one payload parity
...
end. For a clean v0.23 core volume, after the reader has skipped any
...
be zero in v0.23 archives. Readers MUST reject blocks with reserved flag
bits set.
...
No part of the v0.23 write path needs to revisit a closed S3 part or to
...
writing final volumes. There is no conforming v0.23 mode that round-robins
...
### 20.1 Padding overhead (v0.23 unchanged from v0.15)
...
document uses `format_version = 1` and `volume_format_rev = 23`. Readers
...
`volume_format_rev = 23`; accepting earlier draft revisions requires an
...
implementation claims conformance to this v0.23 draft, incompatible
...
### 28.1 Test corpus additions for v0.23
...
- **Volume format revision**: create archives with `volume_format_rev`
  below, equal to, and above 23; verify v0.23-only readers accept only
  23 and reject older or newer revisions.
...
- **Compression registry enforcement**: mutate CryptoHeader to
  `compression_algo = None`; verify v0.23 readers reject it before
...
1. Produces archives whose write sequence is strictly forward
   ...
   `format_version = 1` and `volume_format_rev = 23`.
...
35. Emits v0.23 bootstrap sidecars as packed sequences with no padding,
...
## Appendix A: All changes from v0.22 → v0.23

| Section | Change |
|---|---|
| §8 / §17.1 / §23 / §28.1 / §29 | `volume_format_rev` is bumped to 23 and the stale revision-acceptance test is corrected |
...
| §28.1 / §29 | v0.23 tests and conformance checklist expanded |

---

*End of v0.23 specification.*
