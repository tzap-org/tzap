# tzap Archive Format Specification (v0.32)

| Field | Value |
|---|---|
| **Format version** | 1 |
| **Document version** | 0.32 / 2026-05-22.2 (critical consistency fixes) |
| **Status** | Draft for review, no implementation yet |
| **Owner** | Frank Zhu |
| **Maintainers** | Frank Zhu |
| **Last updated** | 2026-05-22 |
| **Supersedes** | v0.1, v0.2, v0.3, v0.4, v0.5, v0.6, v0.7, v0.8, v0.9, v0.10, v0.11, v0.12, v0.13, v0.14, v0.15, v0.16, v0.17, v0.18, v0.19, v0.20, v0.21, v0.22, v0.23, v0.24, v0.25, v0.26, v0.27, v0.28, v0.29, v0.30, v0.31 |
| **Superseded by** | None |
| **Conflict rule** | This document supersedes earlier tzap format drafts. If it conflicts with v0.1-v0.31 text, this v0.32 draft wins unless a later dated spec explicitly supersedes it. |
| **File extension** | `.tzap` (single-volume) / `.tzap.NNN` (multi-volume) |

## Changelog from v0.31

This revision resolves critical consistency findings in v0.31:
stale wire-revision test guidance, missing directory-hint AEAD counter
uniqueness, underspecified metadata-object zstd framing, and an incomplete
FEC object data ceiling statement.

1. **Draft visibility is bumped.** `volume_format_rev = 32` for this
   draft. (§8, §17.1, §23, §28.1, §29)
2. **Directory-hint AEAD counters are made unique.**
   `DirectoryHintShardEntry.hint_shard_index` values are unique across the
   IndexRoot directory-hint shard table; readers reject duplicates before
   decrypting directory-hint shards. (§14.3, §15.8, §15.9, §22, §28.1,
   §29)
3. **Metadata-object zstd framing is exact.** IndexRoot, IndexShard,
   dictionary, and DirectoryHintTable compressed payloads are exactly one
   complete non-skippable zstd frame that consumes the entire depadded
   plaintext; readers reject trailing bytes, skippable frames, concatenated
   frames, and decompressed-size mismatches. (§14.3, §15.9, §19.1,
   §28.1, §29)
4. **FEC object sizing is made explicit.** A proposed data-block count is
   valid only when its computed parity count fits the class maximum, the
   data/parity total fits the ReedSolomonGF16 limit, and the data-size
   product fits `u32`. (§18, §24, §28.1, §29)
5. **Volume-format revision tests are corrected.** The revision test case
   now targets 32 rather than stale v29/v30 values. (§28.1)
...
a usable archive mode. Conformant v0.32 writers MUST set
`compression_algo = ZstdFramed`; Conformant v0.32 readers MUST reject
...
this draft, not usable archive modes. Conformant v0.32 writers MUST set
`fec_algo = ReedSolomonGF16`; Conformant v0.32 readers MUST reject any
...
- `pad_len = 0` is not valid in v0.32. The extra block in the exact-fit
...
stream), conforming v0.32 writers MUST use `stripe_width = 1`,
...
struct VolumeHeader {
    ...
    volume_format_rev:        u16,       // 32 for this draft
...
parameters.
Readers MUST reject `compression_algo = None` in v0.32; payload and
...
known extensions are single-valued in v0.32; writers MUST NOT emit the
...
| ~~`0x0004`~~ | ~~`[u8; 32]`~~ | **Forbidden in v0.32.** The tar-stream content hash is encrypted inside IndexRoot. Writers MUST NOT emit this extension; readers MUST reject it if present. |
| `0x0005` | UTF-8 | Locale tag for filenames |
| ~~`0x0006`~~ | ~~bytes~~ | **Forbidden in v0.32; moved to encrypted metadata.** A writer setting `has_dictionary = 1` declares that IndexRoot locates a dictionary-object extent (§15.2). Writers MUST NOT emit this extension; readers MUST reject it if present. |
...
struct BlockRecord {
    ...
    flags:         u8,               // bit 0: last data block of encrypted object
                                     // bits 1..7: reserved; MUST be zero in v0.32
    _reserved:     [u8; 2],
...
MUST reject a BlockRecord with any reserved flag bit set; in v0.32 this
...
Completed v0.32 writers MUST set `is_authoritative = 1` in every closed
...
ManifestFooter and VolumeTrailer in v0.32.
...
struct BootstrapSidecarHeader {
    ...
    flags:                       u32,       // bit 0: ManifestFooter present
                                             ...
                                             // bits 3..31: reserved; MUST be zero in v0.32
...
be zero in v0.32; readers MUST reject the sidecar before trusting any
...
ManifestFooter HMAC and `archive_uuid`/`session_id`, MUST reject a v0.32
...
No padding, extension bytes, or unclaimed gaps are permitted in a v0.32
...
non-critical by a future version; for v0.32, unknown flag bits are a hard
error.
...
payload envelope sequence has no gaps. Because v0.32 stores only
...
For every encrypted object in v0.32, `encrypted_size` is the total
...
`FileEntry.flags` is reserved in v0.32. Writers MUST set it to zero, and
...
`(first_dir_hash, hint_shard_index)`. `hint_shard_index` values are
stable AEAD identity counters for directory-hint shard objects; they need
not equal IndexRoot row positions, but they MUST be unique across the
DirectoryHintShardEntry table. Their hash ranges MUST be monotonic: for
adjacent entries,
...
- when the same `frame_index` or `envelope_index` appears in more than
  ...
  version, this is equivalent to byte-identical row encoding in v0.32.
...
1. Read VolumeHeader at offset 0; verify CRC.
   ...
   by the reader. A reader claiming only v0.32 conformance rejects
   `volume_format_rev != 32`. Reject if `VolumeHeader.stripe_width = 0`
...
3. Reject unknown `compression_algo`, `aead_algo`, `fec_algo`, or
   ...
   reader-side cap in v0.32. Parse KdfParams only after `kdf_algo` is
...
In v0.32, archive parameters guarantee at least one payload parity
...
end. For a clean v0.32 core volume, after the reader has skipped any
...
be zero in v0.32 archives. Readers MUST reject blocks with reserved flag
bits set.
...
No part of the v0.32 write path needs to revisit a closed S3 part or to
...
writing final volumes. There is no conforming v0.32 mode that round-robins
...
### 20.1 Padding overhead (v0.32 unchanged from v0.15)
...
document uses `format_version = 1` and `volume_format_rev = 32`. Readers
...
`volume_format_rev = 32`; accepting earlier draft revisions requires an
...
implementation claims conformance to this v0.32 draft, incompatible
...
### 28.1 Test corpus additions through v0.32

Additional v0.32 cases:
...
- **Volume format revision freshness**: create archives with
  `volume_format_rev` below, equal to, and above 32; verify v0.32-only
  readers accept only 32 and reject older or newer revisions.
...
- **CryptoHeader canonical offset**: set `crypto_header_offset` to values
  ...
  with padding bytes before the CryptoHeader; verify v0.32 readers reject
  instead of accepting an unclaimed gap.
- **Volume format revision**: create archives with `volume_format_rev`
  below, equal to, and above 32; verify v0.32-only readers accept only 32
...
- **CryptoHeader Extension TLVs**: with a valid HMAC, inject an unknown
  ...
  verify v0.32 readers ignore it and continue parsing. Repeat with the
...
- **Compression registry enforcement**: mutate CryptoHeader to
  `compression_algo = None`; verify v0.32 readers reject it before
...
1. Produces archives whose write sequence is strictly forward
   ...
   `format_version = 1`, `volume_format_rev = 32`, every fixed magic
...
35. Emits v0.32 bootstrap sidecars as packed sequences with no padding,
...
## Appendix A: All changes from v0.31 -> v0.32

| Section | Change |
|---|---|
| §8 / §17.1 / §23 / §28.1 / §29 | `volume_format_rev` is bumped to 32 |
| §14.3 / §15.8 / §15.9 / §22 / §28.1 / §29 | Requires `DirectoryHintShardEntry.hint_shard_index` values to be unique because they are AEAD counters for the `dirhint` domain |
| §14.3 / §15.9 / §19.1 / §28.1 / §29 | Requires each metadata-object compressed payload to be exactly one complete non-skippable zstd frame consuming the entire depadded plaintext |
| §18 / §24 / §28.1 / §29 | Clarifies that actual encrypted-object data counts must satisfy computed parity, class maxima, the ReedSolomonGF16 total-shard limit, and u32 encrypted-size limits together |
| §28.1 | Corrects the stale volume-format revision test from v29/v30 to v32 |

---

*End of v0.32 specification.*
