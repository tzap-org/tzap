# tzap Archive Format Specification (v0.34)

| Field | Value |
|---|---|
| **Format version** | 1 |
| **Document version** | 0.34 / 2026-05-22.4 (index and FEC wire-profile fixes) |
| **Status** | Draft for review, no implementation yet |
| **Owner** | Frank Zhu |
| **Maintainers** | Frank Zhu |
| **Last updated** | 2026-05-22 |
| **Supersedes** | v0.1, v0.2, v0.3, v0.4, v0.5, v0.6, v0.7, v0.8, v0.9, v0.10, v0.11, v0.12, v0.13, v0.14, v0.15, v0.16, v0.17, v0.18, v0.19, v0.20, v0.21, v0.22, v0.23, v0.24, v0.25, v0.26, v0.27, v0.28, v0.29, v0.30, v0.31, v0.32, v0.33 |
| **Superseded by** | None |
| **Conflict rule** | This document supersedes earlier tzap format drafts. If it conflicts with v0.1-v0.33 text, this v0.34 draft wins unless a later dated spec explicitly supersedes it. |
| **File extension** | `.tzap` (single-volume) / `.tzap.NNN` (multi-volume) |

## Changelog from v0.33

This revision resolves interoperability-significant and lookup-correctness
findings in v0.33: incomplete FEC parity definition, missing AEAD tag
placement, an invalid Argon2id lower bound, and index boundary metadata
that readers could not fully validate against decrypted shard contents.

1. **Draft visibility is bumped.** `volume_format_rev = 34` for this
   draft. (§8, §17.1, §23, §28.1, §29)
2. **ReedSolomonGF16 is wire-complete.** The FEC profile now fixes the
   GF(2^16) primitive polynomial, systematic Cauchy coefficient matrix,
   object-local row identities, parity-symbol formula, and repair matrix
   rule. (§18, §28.1, §29)
3. **AEAD object serialization is canonical.** Every encrypted object is
   serialized as `ciphertext || tag`; detached tags, tag prefixes, and
   implementation-native alternate layouts are non-conforming. (§5, §14,
   §29)
4. **Argon2id rejects zero iterations.** Writers must emit `t_cost >= 1`
   and readers reject `t_cost = 0` before invoking Argon2id. (§13.1,
   §17.1, §28.1, §29)
5. **Index range metadata is validated against actual shard contents.**
   ShardEntry and DirectoryHintShardEntry counts and hash bounds must
   match the decrypted tables they locate; FileEntry and DirectoryHintEntry
   tables must be canonically sorted; sparse local FrameEntry tar offsets
   are clarified. (§15.5, §15.8, §15.9, §28.1, §29)
...
a usable archive mode. Conformant v0.34 writers MUST set
`compression_algo = ZstdFramed`; Conformant v0.34 readers MUST reject
...
this draft, not usable archive modes. Conformant v0.34 writers MUST set
`fec_algo = ReedSolomonGF16`; Conformant v0.34 readers MUST reject any
...
- `pad_len = 0` is not valid in v0.34. The extra block in the exact-fit
...
stream), conforming v0.34 writers MUST use `stripe_width = 1`,
...
struct VolumeHeader {
    ...
    volume_format_rev:        u16,       // 34 for this draft
...
parameters.
Readers MUST reject `compression_algo = None` in v0.34; payload and
...
known extensions are single-valued in v0.34; writers MUST NOT emit the
...
| ~~`0x0004`~~ | ~~`[u8; 32]`~~ | **Forbidden in v0.34.** The tar-stream content hash is encrypted inside IndexRoot. Writers MUST NOT emit this extension; readers MUST reject it if present. |
| `0x0005` | UTF-8 | Locale tag for filenames |
| ~~`0x0006`~~ | ~~bytes~~ | **Forbidden in v0.34; moved to encrypted metadata.** A writer setting `has_dictionary = 1` declares that IndexRoot locates a dictionary-object extent (§15.2). Writers MUST NOT emit this extension; readers MUST reject it if present. |
...
struct BlockRecord {
    ...
    flags:         u8,               // bit 0: last data block of encrypted object
                                     // bits 1..7: reserved; MUST be zero in v0.34
    _reserved:     [u8; 2],
...
MUST reject a BlockRecord with any reserved flag bit set; in v0.34 this
...
Completed v0.34 writers MUST set `is_authoritative = 1` in every closed
...
ManifestFooter and VolumeTrailer in v0.34.
...
struct BootstrapSidecarHeader {
    ...
    flags:                       u32,       // bit 0: ManifestFooter present
                                             ...
                                             // bits 3..31: reserved; MUST be zero in v0.34
...
be zero in v0.34; readers MUST reject the sidecar before trusting any
...
ManifestFooter HMAC and `archive_uuid`/`session_id`, MUST reject a v0.34
...
No padding, extension bytes, or unclaimed gaps are permitted in a v0.34
...
non-critical by a future version; for v0.34, unknown flag bits are a hard
error.
...
payload envelope sequence has no gaps. Because v0.34 stores only
...
For every encrypted object in v0.34, `encrypted_size` is the total
...
`FileEntry.flags` is reserved in v0.34. Writers MUST set it to zero, and
...
extension safety; a v0.34 DirectoryHintShardEntry in IndexRoot MUST NOT
...
- when the same `frame_index` or `envelope_index` appears in more than
  ...
  version, this is equivalent to byte-identical row encoding in v0.34.
...
1. Read VolumeHeader at offset 0; verify CRC.
   ...
   by the reader. A reader claiming only v0.34 conformance rejects
   `volume_format_rev != 34`. Reject if `VolumeHeader.stripe_width = 0`
...
3. Reject unknown `compression_algo`, `aead_algo`, `fec_algo`, or
   ...
   its active reader-side cap in v0.34. Parse KdfParams only after `kdf_algo` is
...
In v0.34, archive parameters guarantee at least one payload parity
...
end. For a clean v0.34 core volume, after the reader has skipped any
...
be zero in v0.34 archives. Readers MUST reject blocks with reserved flag
bits set.
...
No part of the v0.34 write path needs to revisit a closed S3 part or to
...
writing final volumes. There is no conforming v0.34 mode that round-robins
...
### 20.1 Padding overhead (v0.34 unchanged from v0.15)
...
document uses `format_version = 1` and `volume_format_rev = 34`. Readers
...
`volume_format_rev = 34`; accepting earlier draft revisions requires an
...
implementation claims conformance to this v0.34 draft, incompatible
...
### 28.1 Test corpus additions through v0.34

Additional v0.34 cases:
...
- **Volume format revision freshness**: create archives with
  `volume_format_rev` below, equal to, and above 34; verify v0.34-only
  readers accept only 34 and reject older or newer revisions.
...
- **CryptoHeader canonical offset**: set `crypto_header_offset` to values
  ...
  with padding bytes before the CryptoHeader; verify v0.34 readers reject
  instead of accepting an unclaimed gap.
- **Volume format revision**: create archives with `volume_format_rev`
  below, equal to, and above 34; verify v0.34-only readers accept only 34
...
- **CryptoHeader Extension TLVs**: with a valid HMAC, inject an unknown
  ...
  verify v0.34 readers ignore it and continue parsing. Repeat with the
...
- **Compression registry enforcement**: mutate CryptoHeader to
  `compression_algo = None`; verify v0.34 readers reject it before
...
1. Produces archives whose write sequence is strictly forward
   ...
   `format_version = 1`, `volume_format_rev = 34`, every fixed magic
...
35. Emits v0.34 bootstrap sidecars as packed sequences with no padding,
...
## Appendix A: All changes from v0.33 -> v0.34

| Section | Change |
|---|---|
| §8 / §17.1 / §23 / §28.1 / §29 | `volume_format_rev` is bumped to 34 |
| §5 / §14 / §28.1 / §29 | Defines AEAD encrypted-object serialization as `ciphertext || tag` |
| §13.1 / §17.1 / §28.1 / §29 | Requires Argon2id `t_cost ≥ 1` and reader rejection of `t_cost = 0` before KDF execution |
| §15.5 / §15.8 / §15.9 / §28.1 / §29 | Requires ShardEntry and DirectoryHintShardEntry count/hash-bound fields to match decrypted shard tables; requires canonical FileEntry and DirectoryHintEntry ordering; clarifies sparse local FrameEntry tar-offset validation |
| §18 / §28.1 / §29 | Defines the exact `ReedSolomonGF16` GF(2^16) Cauchy wire profile, including polynomial, coefficients, row identities, parity formula, and repair matrix rule |

---

*End of v0.34 specification.*
