# tzap Archive Format Specification (v0.18)

| Field | Value |
|---|---|
| **Format version** | 1 |
| **Document version** | 0.18 / 2026-05-21.2 (draft after verification and resource-cap review) |
| **Status** | Draft for review, no implementation yet |
| **Owner** | Frank Zhu |
| **Maintainers** | Frank Zhu |
| **Last updated** | 2026-05-21 |
| **Supersedes** | v0.1, v0.2, v0.3, v0.4, v0.5, v0.6, v0.7, v0.8, v0.9, v0.10, v0.11, v0.12, v0.13, v0.14, v0.15, v0.16, v0.17 |
| **Superseded by** | None |
| **Conflict rule** | This document supersedes earlier tzap format drafts. If it conflicts with v0.1-v0.17 text, this v0.18 draft wins unless a later dated spec explicitly supersedes it. |
| **File extension** | `.tzap` (single-volume) / `.tzap.NNN` (multi-volume) |

## Changelog from v0.17

This revision closes verification, structural-validation, and resource-cap
gaps found during external review.

1. **Draft visibility is bumped.** `volume_format_rev = 18` for this
   draft. (§8, §17.1, §23)
2. **Full-archive verification now validates all IndexRoot totals.**
   `content_sha256`, `file_count`, `frame_count`, `envelope_count`,
   `payload_block_count`, and `tar_total_size` are all cross-checked
   against distinct shard-local rows and reconstructed tar bytes. (§15.9,
   §29)
3. **Global tar-stream contiguity is required by `verify`.** Distinct
   FrameEntry rows must cover `[0, tar_total_size)` without gaps or
   overlap. (§15.9)
4. **Index table layouts are canonical and non-overlapping.** IndexRoot,
   IndexShard, and DirectoryHintTable counted regions are validated with
   packed cursor rules before indexing or allocation. (§15.9)
5. **Directory hint shard identity is cross-checked.**
   `DirectoryHintTable.hint_shard_index` must match the locating
   DirectoryHintShardEntry. (§15.9)
6. **Path normalization is explicit.** FileEntry paths use NFC UTF-8 and
   `/` as the only archive component separator; lookup hashes are computed
   over normalized path bytes. (§15.6, §15.7, §16)
7. **Reader caps now cover sidecars and directory hints.** Bootstrap
   sidecar size, sidecar BlockRecord copies, hint-shard count, and
   per-hint-shard entry count are bounded. (§12.3, §13.3, §15.9)
8. **Total extraction size is defined.** The cap applies to logical bytes
   that would be written for selected regular-file payloads after tar/PAX
   interpretation. (§13.3)
9. **Sequential extraction stop and failure behavior are explicit.**
   Readers scan BlockRecords until the first non-`TZBK` record, skip
   metadata-object BlockRecords, and must not continue after an
   irrecoverable envelope. (§17.3)
10. **`CompressionAlgo::None` is reserved, not usable in this draft.**
    v0.18 archives must use `ZstdFramed`. (§5)
11. **Streaming writers must choose FEC class maxima before CryptoHeader
    emission.** Unknown-size streaming writers may choose class maxima
    upfront or reject/spool; they cannot raise header values later. (§19,
    §24)
12. **Exact-fit padding overflow is forbidden.** Writers must split before
    the mandatory extra padding block would exceed u32 or FEC limits.
    (§6.1, §14.2, §29)
13. **Smaller consistency fixes.** `is_authoritative = 1` is in writer
    conformance, CLI parity behavior is separated from wire-format
    parity requirements, `closed_at_ns` is listed as leaked plaintext
    metadata, `file_data_size` is validated against tar metadata when
    data is decoded, `ShardEntry.shard_index` is unique, stale historical
    labels are neutralized, and IndexRoot's reserved tail is a single
    32-byte field. (§11, §15, §22, §29)
...
a usable archive mode. Conformant v0.18 writers MUST set
`compression_algo = ZstdFramed`; conformant v0.18 readers MUST reject
...
- `pad_len = 0` is not valid in v0.18. The extra block in the exact-fit
...
stream), conforming v0.18 writers MUST use `stripe_width = 1`,
...
struct VolumeHeader {
    ...
    volume_format_rev:        u16,       // 18 for this draft
...
Readers MUST reject `compression_algo = None` in v0.18; payload and
...
| ~~`0x0004`~~ | ~~`[u8; 32]`~~ | **Forbidden in v0.18.** The tar-stream content hash is encrypted inside IndexRoot. Writers MUST NOT emit this extension; readers MUST reject it if present. |
...
struct BlockRecord {
    ...
    flags:         u8,               // bit 0: last data block of encrypted object
                                     // bits 1..7: reserved; MUST be zero in v0.18
    _reserved:     [u8; 2],
...
MUST reject a BlockRecord with any reserved flag bit set; in v0.18 this
...
Completed v0.18 writers MUST set `is_authoritative = 1` in every closed
...
For seekable inputs, authenticated `bytes_written` MUST equal
`file_size - sizeof(VolumeTrailer)`. Readers MUST reject a seekable
volume whose trailer HMAC is valid but whose `bytes_written` does not
match the observed file length.
...
`archive_uuid`/`session_id`, but MUST NOT require its `volume_index` to
...
No padding, extension bytes, or unclaimed gaps are permitted in a v0.18
...
non-critical by a future version; for v0.18, unknown flag bits are a hard
error.
...
```
128
+ sizeof(ManifestFooter)
+ 2 × (index_root_fec_data_shards + index_root_fec_parity_shards)
      × sizeof(BlockRecord)
```

The `sizeof(ManifestFooter)` term is included only when flag bit 0 is
set; the two BlockRecord terms are upper bounds for IndexRoot and
dictionary record copies. Readers MAY expose a lower local cap, but MUST
reject a sidecar whose declared packed size or observed file size exceeds
...
For every encrypted object in v0.18, `encrypted_size` is the total
...
`FileEntry.flags` is reserved in v0.18. Writers MUST set it to zero, and
...
- an IndexRoot with `shard_count > 0` has `shard_table_offset !=
  sizeof(IndexRoot)`, or an IndexRoot with `directory_hint_shard_count >
  0` has `directory_hint_shard_table_offset != shard_table_offset +
  shard_count × sizeof(ShardEntry)`;
- an IndexRoot plaintext length is not exactly
  `sizeof(IndexRoot) + shard_count × sizeof(ShardEntry) +
  directory_hint_shard_count × sizeof(DirectoryHintShardEntry)`;
- an IndexShard plaintext is not packed exactly as
  `IndexShardHeader`, present FileEntry table, present FrameEntry table,
  present EnvelopeEntry table, then present string pool, with no gaps or
  overlap;
...
- `shard_count`, `directory_hint_shard_count`, `envelope_count`,
  `frame_count`, or `file_count` exceed reader resource caps;
...
For the DirectoryHintTable packed-layout rule, the referenced u32
shard-ID array length is the smallest byte length that covers every
`DirectoryHintEntry.shard_list_start_index + shard_count` range, computed
with checked arithmetic and multiplied by 4. If `entry_count > 0`,
...
the final cursor is the end of the shard-ID array.
...
- for each payload EnvelopeEntry, the sum of `compressed_size` for the
  FrameEntry rows in its frame range equals `EnvelopeEntry.plaintext_size`;
  those FrameEntry ranges MUST cover the envelope plaintext exactly once
  without gaps or overlap when ordered by `offset_in_envelope`;
...
- when the same `frame_index` or `envelope_index` appears in more than
  ...
  version, this is equivalent to byte-identical row encoding in v0.18.
...
1. Read VolumeHeader at offset 0; verify CRC.
   ...
   only v0.18 conformance rejects `volume_format_rev != 18`.
...
6. If the input is seekable:
     ...
     c. Verify trailer magic and trailer HMAC. On failure: this volume is
        ...
        `volume_index` match the VolumeHeader, and that
        `bytes_written == file_size - sizeof(VolumeTrailer)`; on
...
7. If the input is non-seekable:
     a. If a trusted bootstrap sidecar is supplied, use it for
        ...
        `total_volumes == CryptoHeader.stripe_width`; do not compare its
        `volume_index` with the current VolumeHeader because the sidecar
        is not a volume.
...
11. If has_dictionary = 1: read the dictionary object blocks from the
    ...
    `dictionary_key` using domain `dict`, strip suffix padding, and
    zstd-decompress (no dictionary). Initialize the payload zstd
    decompression context with those bytes.
...
19. Slice and zstd-decode each complete compressed frame from its
    ...
    reporting the size as content-verified.
```
...
be zero in v0.18 archives. Readers MUST reject blocks with reserved flag
bits set.
...
No part of the v0.18 write path needs to revisit a closed S3 part or to
...
writing final volumes. There is no conforming v0.18 mode that round-robins
...
### 20.1 Padding overhead (v0.18 unchanged from v0.15)
...
document uses `format_version = 1` and `volume_format_rev = 18`. Readers
...
`volume_format_rev = 18`; accepting earlier draft revisions requires an
...
implementation claims conformance to this v0.18 draft, incompatible
...
| `index_root_fec_data_shards` | dynamic before CryptoHeader emission, minimum 16 | maximum IndexRoot and dictionary-object data blocks; writer MUST choose a value large enough for serialized IndexRoot before writing CryptoHeader, but total data+parity shards and `data_shards * block_size` must fit §18 limits |
...
### 28.1 Test corpus additions for v0.18
...
- **Volume format revision**: create archives with `volume_format_rev`
  below, equal to, and above 18; verify v0.18-only readers accept only
  18 and reject newer revisions.
...
- **Chunk-size semantics**: archives with `chunk_size` smaller than,
  ...
  extents remain the only random-access authority.
- **Compression registry enforcement**: mutate CryptoHeader to
  `compression_algo = None`; verify v0.18 readers reject it before
...
- **Structural validation**: malformed IndexRoot and IndexShard buffers
  ...
  gaps, non-canonical IndexRoot table offsets, and out-of-range
  string-pool paths; verify rejection before allocation. Include an
  IndexShard whose
...
- **Archive totals**: mutate `IndexRoot.payload_block_count` so it no
  ...
  after reconstructing the tar stream.
...
- **Streaming IndexRoot FEC preselection**: feed a true streaming writer
  ...
  CryptoHeader after payload metadata is known.
...
- **Sequential scan boundary**: append index, dictionary, and
  ...
  irrecoverably and verify the reader does not attempt later envelopes.
- **Bootstrap sidecar**: dictionary archive with a bootstrap sidecar;
  ...
  another `volume_index`. Verify sparse flag combinations follow the
  §12.3 cursor rule. Verify sidecars with padding, extension bytes,
...
- **Trailer byte count**: mutate authenticated `VolumeTrailer.bytes_written`
  so it no longer equals `file_size - 128`; verify seekable readers
  reject after trailer HMAC verification.
...
1. Produces archives whose write sequence is strictly forward
   ...
   `format_version = 1` and `volume_format_rev = 18`.
...
34. Sets all IndexRoot dictionary fields to zero whenever
    `has_dictionary = 0`.
35. Emits v0.18 bootstrap sidecars as packed sequences with no padding,
...
37. Sets `IndexRoot.file_count`, `frame_count`, `envelope_count`,
    ...
    the exact reconstructed tar stream.
...
16. Validates §12.3 sidecar HMAC, UUID/session binding,
    ...
    sidecar ManifestFooter, does not compare `volume_index` with the
...
18. Rejects `compression_algo = None`, rejects `chunk_size = 0`, and
    treats non-zero `chunk_size` as advisory metadata only; FrameEntry
    and EnvelopeEntry remain authoritative.
...
24. Verifies authenticated VolumeTrailer and per-volume ManifestFooter
    ...
    verifies seekable `bytes_written == file_size - 128`, and
...
29. Rejects `has_dictionary = 0` with non-zero IndexRoot dictionary
    ...
    offsets, IndexShard `shard_index` mismatches, DirectoryHintTable
    `hint_shard_index` mismatches, duplicate ShardEntry `shard_index`
...
31. During full-archive `verify`, checks that
    ...
    the distinct shard-local rows and the exact reconstructed tar stream,
    including global tar-stream coverage with no gap or overlap.
...
## Appendix A: All changes from v0.17 → v0.18

| Section | Change |
|---|---|
| §5 / §9 / §17.1 / §29 | `CompressionAlgo::None` reserved; v0.18 archives must use `ZstdFramed` |
| §6.1 / §14.2 / §19 / §29 | Exact-fit padding extra block must not overflow u32 or FEC object limits |
| §10 | Partial-volume object reads must prove available/missing shard positions are repairable before AEAD |
| §12.3 / §13.3 | Bootstrap sidecar size and copied BlockRecord sections capped |
| §13.3 / §15.8 / §15.9 | Directory-hint shard and entry counts capped |
| §13.3 | Total extraction size cap defined as logical regular-file payload bytes after tar/PAX interpretation |
| §15.2 / §15.9 / §29 | IndexRoot totals and `content_sha256` verified against distinct rows and reconstructed tar stream |
| §15.4 / §15.9 / §29 | ShardEntry `shard_index` uniqueness required |
| §15.5 / §15.9 | IndexRoot, IndexShard, and DirectoryHintTable table layouts made canonical, packed, and non-overlapping |
| §15.6 / §15.7 / §16 | FileEntry path separator and lookup normalization made explicit |
| §15.6 / §15.9 / §17.2 / §29 | `file_data_size` bound to decoded tar member size when content is decoded |
| §15.8 / §15.9 / §29 | DirectoryHintTable `hint_shard_index` cross-check required |
| §17.3 | Sequential extraction stop condition and irrecoverable-envelope behavior clarified |
| §19 / §24 / §29 | Streaming writers must choose IndexRoot FEC class maxima before CryptoHeader HMAC |
| §22 | `closed_at_ns` added to plaintext leakage inventory |
| §28.1 / §29 | v0.18 tests and conformance checklist expanded |

---

*End of v0.18 specification.*
