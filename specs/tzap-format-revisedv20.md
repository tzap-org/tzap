# tzap Archive Format Specification (v0.20)

| Field | Value |
|---|---|
| **Format version** | 1 |
| **Document version** | 0.20 / 2026-05-21.4 (draft after collision lookup and active-tamper availability review) |
| **Status** | Draft for review, no implementation yet |
| **Owner** | Frank Zhu |
| **Maintainers** | Frank Zhu |
| **Last updated** | 2026-05-21 |
| **Supersedes** | v0.1, v0.2, v0.3, v0.4, v0.5, v0.6, v0.7, v0.8, v0.9, v0.10, v0.11, v0.12, v0.13, v0.14, v0.15, v0.16, v0.17, v0.18, v0.19 |
| **Superseded by** | None |
| **Conflict rule** | This document supersedes earlier tzap format drafts. If it conflicts with v0.1-v0.19 text, this v0.20 draft wins unless a later dated spec explicitly supersedes it. |
| **File extension** | `.tzap` (single-volume) / `.tzap.NNN` (multi-volume) |

## Changelog from v0.19

This revision tightens hash-collision lookup, object-size validation,
and operational recovery wording found during external review.

1. **Draft visibility is bumped.** `volume_format_rev = 20` for this
   draft. (§8, §17.1, §23)
2. **Hash-prefix lookup is now deterministic under overlapping equal
   prefixes.** Shard and directory-hint lookup use an explicit
   upper-bound-on-first-hash landing rule, then scan the bounded
   contiguous candidate block including the landing entry. (§15.4,
   §15.7, §15.8)
3. **Padding marker edge cases are made explicit.** A final `0xFF` byte is
   always wide form, `N < 5` rejects immediately, and `pad_len = 0` is
   malformed. (§6.1)
4. **Object class-limit checks are enumerated.** Payload envelopes,
   IndexShards, directory-hint shards, dictionary objects, and IndexRoot
   all state their CryptoHeader class maxima and GF16 total-shard checks
   explicitly. (§15.9, §17.2)
5. **Sidecar validation is stricter and safer.** Sidecar cap arithmetic
   now rejects values exceeding host file-size/addressability limits, the
   cap accounts for present record sections, and sidecar ManifestFooter
   copies must have `volume_index = 0`. (§12.3, §13.3, §17.1)
6. **Active tamper availability is scoped honestly.** CRC/FEC repairs
   accidental corruption and missing-volume erasures; an active attacker
   who can recompute unkeyed CRCs can still force AEAD failure and data
   unavailability without plaintext exposure. (§3, §18, §22)
7. **Trailing-garbage recovery is normative but bounded.** Writers still
   close volumes with the trailer as the final bytes; readers may scan
   backward within a cap and validate `bytes_written` against the
   candidate trailer offset. (§13.3, §17.1, §21)
8. **Operational streaming failure is called out.** Unknown-cardinality
   streaming writers must pre-scan, spool, choose sufficient maxima, or
   accept that final IndexRoot overflow rejects the archive. (§19.3,
   §21)
9. **Smaller clarity fixes.** `max_hash_prefix_run_files` is labeled as a
   writer-side default, envelope frame coverage rejection is restated,
   and security notes describe collision-run lookup failures. (§15.9,
   §22, §24)
...
a usable archive mode. Conformant v0.20 writers MUST set
`compression_algo = ZstdFramed`; conformant v0.20 readers MUST reject
...
- `pad_len = 0` is not valid in v0.20. The extra block in the exact-fit
...
stream), conforming v0.20 writers MUST use `stripe_width = 1`,
...
struct VolumeHeader {
    ...
    volume_format_rev:        u16,       // 20 for this draft
...
Readers MUST reject `compression_algo = None` in v0.20; payload and
metadata compression in this draft is always `ZstdFramed`.
...
`block_size < 4096`. Readers MUST also reject `chunk_size`,
`envelope_target_size`, or `block_size` values above their configured
reader-side caps (§13.3) before allocating buffers or planning work.
...
| ~~`0x0004`~~ | ~~`[u8; 32]`~~ | **Forbidden in v0.20.** The tar-stream content hash is encrypted inside IndexRoot. Writers MUST NOT emit this extension; readers MUST reject it if present. |
...
struct BlockRecord {
    ...
    flags:         u8,               // bit 0: last data block of encrypted object
                                     // bits 1..7: reserved; MUST be zero in v0.20
    _reserved:     [u8; 2],
...
MUST reject a BlockRecord with any reserved flag bit set; in v0.20 this
...
Readers MUST reject parity blocks with bit 0 set.
...
Completed v0.20 writers MUST set `is_authoritative = 1` in every closed
...
`index_root_data_block_count * block_size` overflows u32 or does not
equal `index_root_encrypted_size` before fetching or decrypting
IndexRoot. This is the IndexRoot instance of the encrypted-object size
canonicality rule used throughout §15.9.
...
struct BootstrapSidecarHeader {
    ...
    flags:                       u32,       // bit 0: ManifestFooter present
                                             ...
                                             // bits 3..31: reserved; MUST be zero in v0.20
...
When a ManifestFooter is copied into a bootstrap sidecar, writers MUST
store `ManifestFooter.volume_index = 0`. That value is informational for
sidecar bootstrapping because the sidecar is not itself a volume.
Readers MUST verify the sidecar ManifestFooter HMAC and
`archive_uuid`/`session_id`, MUST reject a v0.20 bootstrap sidecar whose
copied ManifestFooter has `volume_index != 0`, and MUST NOT require that
zero value to match the currently opened VolumeHeader. Seekable
per-volume ManifestFooter copies still MUST match their containing
volume (§17.1).
...
No padding, extension bytes, or unclaimed gaps are permitted in a v0.20
...
non-critical by a future version; for v0.20, unknown flag bits are a hard
error.
...
`(SHA-256(normalized path)[0..8], normalized path bytes)`,** not alphabetically by
...
For every encrypted object in v0.20, `encrypted_size` is the total
...
`(SHA-256(normalized path)[0..8], normalized path bytes)`, shards are contiguous
...
readers apply the candidate-block scan below.
...
for the first ShardEntry whose `first_path_hash > target_hash`; step back
one row to the landing candidate. If no such previous row exists, or if
the landing candidate's `last_path_hash < target_hash`, no shard contains
the hash. Otherwise, collect the contiguous candidate block including the
landing row by scanning left and right while
...
// Then:
//   FileEntry[file_count]   sorted by (SHA-256(normalized path)[0..8], path bytes)
...
`FileEntry.flags` is reserved in v0.20. Writers MUST set it to zero, and
...
verification MUST compare the tar-declared logical payload size with
`FileEntry.file_data_size` and reject on mismatch before reporting the
size as content-verified. Index-only listing tools MAY display
`file_data_size` as authenticated index metadata, but MUST NOT label it
as verified against the tar stream unless this check has run.
...
3. Binary search ShardEntry[] with the §15.4 upper-bound rule: find the
   first row whose `first_path_hash > target_hash`, step back to the
   landing candidate, reject if that candidate's `last_path_hash <
   target_hash`, and otherwise collect the contiguous candidate block
   including the landing row while
...
struct DirectoryHintEntry {
    ...
    path_length:            u32,
    _reserved:              u32,
    shard_list_start_index: u32,        // u32 index into shard-ID array
    shard_count:            u32,
    _reserved2:             u64,
}
...
MUST be 4-byte aligned. `DirectoryHintEntry.shard_list_start_index` is a
u32 element index into that shard-ID array, not a byte offset. The
`shard_count` IDs for an entry start at
...
Every shard ID MUST be `< IndexRoot.shard_count`. Readers MUST perform
the multiplication and addition with checked arithmetic.
...
reader resource caps. The 64-bit offsets
...
2. Binary-search `DirectoryHintShardEntry[]` in IndexRoot with the §15.4
   ...
   `first_dir_hash > dir_hash`, step back to the landing candidate,
   reject if that candidate's `last_dir_hash < dir_hash`, and otherwise
   collect the contiguous candidate block including the landing row while
...
3. For each matching hint shard, read its object-local block extent,
   ...
   checked arithmetic and verifying the counts fit the index FEC class
   limits. Then FEC-repair if needed, AEAD-decrypt with `dir_hint_key`,
   and zstd-decompress without a dictionary.
...
5. Use the associated sorted u32 shard-ID list as the candidate
   IndexShard ID set.
6. For each shard ID, locate the ShardEntry in the IndexRoot shard table
   whose `shard_index` equals that ID, then use that ShardEntry's
...
- a DirectoryHintTable plaintext is not packed exactly as
  ...
  contiguous u32 shard-ID array, then present string pool, with no gaps
  or overlap;
...
- any DirectoryHintEntry shard list range overflows, or
  `shard_list_start_index + shard_count` exceeds the number of u32 IDs
  available in the DirectoryHintTable shard-list array;
...
that directory hints point to IndexShard IDs; hint shards without any
IndexShards are malformed.
...
referenced u32 shard-ID array length is the smallest byte length that
covers every `DirectoryHintEntry.shard_list_start_index + shard_count`
range, computed with checked arithmetic and multiplied by 4. In that
case `entry_table_offset` MUST equal `sizeof(DirectoryHintTable)` and
...
computed shard-ID array; otherwise `string_pool_offset` MUST be zero and
the final cursor is the end of the shard-ID array. DirectoryHintTable
plaintext length MUST equal the applicable final cursor.
...
- whenever a FileEntry tar member group is decoded, `file_data_size`
  matches the logical payload size declared by the main tar entry after
  supported PAX/GNU size metadata is applied, and is zero for
  non-regular entries;
...
- when the same `frame_index` or `envelope_index` appears in more than
  ...
  version, this is equivalent to byte-identical row encoding in v0.20.
...
1. Read VolumeHeader at offset 0; verify CRC.
   ...
   only v0.20 conformance rejects `volume_format_rev != 20`.
...
3. Parse KdfParams; prompt for passphrase or load keyfile. If
   ...
   `m_cost_kib < 8 × parallelism`. Reject `compression_algo !=
   ZstdFramed` in v0.20.
...
6. If the input is seekable:
     ...
     d. Range-check `manifest_footer_offset` and
        ...
        checked arithmetic before using the IndexRoot extent.
...
7. If the input is non-seekable:
     a. If a trusted bootstrap sidecar is supplied, use it for
        ...
        `volume_index = 0` for the sidecar copy, but do not compare that
        value with the current VolumeHeader because the sidecar is not a
        volume. Also verify
        `index_root_encrypted_size = index_root_data_block_count *
        block_size` with checked arithmetic before using the IndexRoot
        extent.
...
9. Read IndexRoot data and parity blocks using
   ...
   `index_root_encrypted_size = index_root_data_block_count *
   block_size` with checked arithmetic. FEC-repair, trim to
   index_root_encrypted_size, AEAD-decrypt with index_root_key, strip
   suffix padding, and zstd-decompress (no dictionary).
...
11. If has_dictionary = 1: read the dictionary object blocks from the
    ...
    block_size` with checked arithmetic and verify dictionary
    data/parity counts fit the IndexRoot FEC class limits. Then repair,
    trim, AEAD-decrypt with `dictionary_key` using domain `dict`, strip
    suffix padding, and zstd-decompress (no dictionary). Initialize the
    payload zstd decompression context with those bytes.
...
14. Read candidate shard data/parity blocks using each ShardEntry's
    ...
    arithmetic and verify the counts fit the index-shard FEC class
    limits. Then repair, trim to encrypted_size, AEAD-decrypt with
    index_shard_key, strip suffix padding, and zstd-decompress
    (no dictionary).
...
18. For each needed envelope, read its data and parity blocks using the
    ...
    verify `encrypted_size = data_block_count * block_size` with checked
    arithmetic and verify the counts fit the payload-envelope FEC class
    limits. Then repair if needed, trim to encrypted_size, AEAD-decrypt
    with enc_key, and strip suffix padding. Verify the depadded plaintext
    length equals `EnvelopeEntry.plaintext_size`; reject before slicing
    if it differs.
19. Slice and zstd-decode each complete compressed frame from its
    ...
    the main tar entry's logical payload size with
    `FileEntry.file_data_size` before writing file payload bytes or
    reporting the size as content-verified. If the tar consumer requires
    a POSIX end-of-archive marker, append two synthetic 512-byte zero
    blocks after the selected tar member group bytes; do not count those
    bytes in any IndexRoot or FileEntry size/hash field.
```
...
while the next 4 bytes are `TZBK`. The first non-`TZBK` magic after a
complete BlockRecord boundary stops sequential payload scanning; it is
expected to be ManifestFooter/trailer material or end-of-stream. A
truncated partial BlockRecord is malformed unless an explicit recovery
mode can repair the affected object. BlockRecords of kinds 2 through 9
may appear after payload blocks in the same stream; a sequential
extractor that is not bootstrapping metadata MUST validate their
BlockRecord framing/CRC and skip them, not treat their presence as an
envelope-counter event or as an abort condition. Payload-parity
BlockRecords are skipped unless repair is requested.
...
position by inspecting that block. If FEC parity cannot reconstruct
enough data blocks for the current envelope to authenticate, the reader
MUST abort sequential extraction at that envelope. It MUST NOT guess the
envelope boundary.
...
After a complete sequential extraction reaches the non-`TZBK` boundary
or end-of-stream, readers that feed the decoded stream to a strict tar
consumer MUST append two synthetic 512-byte zero blocks. These bytes are
not archive content and are not included in `tar_total_size` or
`content_sha256`.
...
be zero in v0.20 archives. Readers MUST reject blocks with reserved flag
bits set.
...
No part of the v0.20 write path needs to revisit a closed S3 part or to
...
writing final volumes. There is no conforming v0.20 mode that round-robins
...
### 20.1 Padding overhead (v0.20 unchanged from v0.15)
...
document uses `format_version = 1` and `volume_format_rev = 20`. Readers
...
`volume_format_rev = 20`; accepting earlier draft revisions requires an
...
implementation claims conformance to this v0.20 draft, incompatible
...
### 28.1 Test corpus additions for v0.20
...
- **Shard and collision caps**: create a shard over
  ...
  shard, and returns the complete bounded candidate block.
...
- **Volume format revision**: create archives with `volume_format_rev`
  below, equal to, and above 20; verify v0.20-only readers accept only
  20 and reject newer revisions.
...
- **Chunk-size semantics**: archives with `chunk_size` smaller than,
  ...
  values for parsing, readers reject `chunk_size = 0`, and FrameEntry
  extents remain the only random-access authority. Also verify readers
  reject `chunk_size`, `envelope_target_size`, and `block_size` values
  above configured caps before allocation.
- **Compression registry enforcement**: mutate CryptoHeader to
  `compression_algo = None`; verify v0.20 readers reject it before
  payload or metadata decompression.
...
- **Directory hint sharding**: build directory hints whose entry table
  ...
  shard-ID range and reject ranges whose checked
  `start_index + shard_count` exceeds the array length. Also verify
...
- **Directory hint lookup**: directory paths at root and nested levels,
  ...
  then resolves each ID through the IndexRoot ShardEntry table before
  reading the IndexShard.
...
- **File data size binding**: mutate `FileEntry.file_data_size` so it no
  longer matches the main tar entry's logical size for a regular file,
  and mutate a non-regular entry to a non-zero value; verify extraction
  and full-archive `verify` reject once the tar member group is decoded.
...
- **Sequential scan boundary**: append index, dictionary, and
  ...
  BlockRecords, stops at the first non-`TZBK` boundary, and does not
  increment the envelope counter for skipped kinds. Corrupt one envelope
  ...
  does not attempt later envelopes when FEC cannot repair it.
- **Bootstrap sidecar**: dictionary archive with a bootstrap sidecar;
  ...
  `volume_index = 0` and can bootstrap while the opened VolumeHeader has
  another `volume_index`; mutate that copied value to non-zero and
  verify rejection. Verify sparse flag combinations follow the §12.3
  cursor rule. Verify sidecars with padding, extension bytes,
  unclaimed gaps, or trailing bytes are rejected.
...
1. Produces archives whose write sequence is strictly forward
   ...
   `format_version = 1` and `volume_format_rev = 20`.
...
7. Stores the ManifestFooter pointer in the VolumeTrailer and emits a
   ...
   closed completed volume. Writes no bytes after the VolumeTrailer.
...
20. Sets `compression_algo = ZstdFramed`, and sets `chunk_size` to a
    non-zero writer target no larger than
...
22. Sets all reserved BlockRecord flag bits to zero, sets bit 0 on and
    only on the last data block of each encrypted object, and never sets
    bit 0 on parity blocks.
...
25. Sets `FileEntry.flags = 0` and emits only NFC UTF-8 archive paths
    ...
    `file_data_size` that matches the main tar entry's logical payload
    size.
...
28. Never emits a payload envelope with `frame_count = 0`,
    ...
    FrameEntry with zero compressed or decompressed size.
...
35. Emits v0.20 bootstrap sidecars as packed sequences with no padding,
    extension bytes, unclaimed gaps, or trailing bytes. If a ManifestFooter
    is copied into a sidecar, sets that copied footer's `volume_index = 0`.
...
1. On seekable input, locates the VolumeTrailer by seeking to
   `file_size - 128`, with only the bounded trailing-garbage recovery
   scan allowed by §17.1.
...
7. Searches the file table by
   ...
   string compare on partial path bounds.
8. Uses the §15.4 upper-bound-on-`first_path_hash` lookup rule, includes
   the landing shard in the candidate block, and scans adjacent
   containing shards subject to `max_hash_collision_shard_scan`.
...
11. Uses object-local FEC counts from ManifestFooter, EnvelopeEntry,
   ...
   data_block_count * block_size` and class limits with checked
   arithmetic.
...
16. Validates §12.3 sidecar HMAC, UUID/session binding,
    ...
    sidecar ManifestFooter, requires `volume_index = 0` and does not
    compare that value with the current VolumeHeader.
...
18. Rejects `compression_algo = None`, rejects `chunk_size = 0`,
    rejects `chunk_size`, `envelope_target_size`, or `block_size` values
    above configured reader caps, and then treats the validated
    `chunk_size` as advisory metadata only; FrameEntry and EnvelopeEntry
...
20. For sequential extraction, derives each payload envelope AEAD counter
    ...
    after a complete payload envelope authenticates. After an
    irrecoverable envelope failure, does not continue by guessing a
    counter or boundary.
...
23. Rejects non-zero `FileEntry.flags`, unsafe archive paths, and
    ...
    zero compressed/decompressed size, FileEntry tar member groups whose
    main tar entry size does not match `file_data_size` when decoded, and
...
29. Rejects `has_dictionary = 0` with non-zero IndexRoot dictionary
    fields, `has_dictionary = 1` with missing or zero required
    ...
    values, invalid DirectoryHintShardEntry ordering or shard-list
    ranges, and non-matching duplicate FrameEntry/EnvelopeEntry rows
    when observed.
...
## Appendix A: All changes from v0.19 → v0.20

| Section | Change |
|---|---|
| §3 / §18 / §21 / §22 / §28.1 | Clarified that unkeyed BlockRecord CRC supports accidental-corruption repair only; active tamper with recomputed CRC is detected by AEAD/HMAC but may deny FEC availability |
| §6.1 / §28.1 / §29 | Padding edge cases now explicitly reject final `0xFF` with `N < 5`, `pad_len = 0`, and any attempted byte-form `pad_len = 255` interpretation |
| §12 / §17.1 / §21 / §28.1 / §29 | Conforming writers emit no bytes after VolumeTrailer; readers may use bounded authenticated trailing-garbage recovery |
| §12.3 / §13.3 / §17.1 / §28.1 / §29 | Bootstrap sidecar cap counts only present record sections, uses checked 64-bit-or-wider arithmetic with host-size rejection, and sidecar ManifestFooter copies must have `volume_index = 0` |
| §15.4 / §15.7 / §15.8 / §22 / §28.1 / §29 | Hash-prefix shard lookup now uses deterministic upper-bound landing and candidate-block scanning for overlapping equal-prefix shard ranges |
| §15.8 / §17.2 / §29 | Directory hint, index shard, dictionary, IndexRoot, and payload object reads restate `encrypted_size = data_block_count * block_size` and class-limit checks before FEC repair |
| §15.9 / §29 | Per-object data/parity class maxima and ReedSolomonGF16 total-shard checks are explicit for EnvelopeEntry, ShardEntry, DirectoryHintShardEntry, IndexRoot, and dictionary objects |
| §15.9 / §28.1 | Payload envelope FrameEntry ranges must explicitly cover `[0, plaintext_size)` exactly once, without gaps or overlap |
| §19.3 / §21 / §24 / §28.1 | Streaming writers must reject finalization if the non-splittable IndexRoot outgrows already-authenticated class limits; operational risk is documented |
| §24 | `max_hash_prefix_run_files` documented as a writer-side planning default, not a reader validation cap |
| §28.1 / §29 | v0.20 tests and conformance checklist expanded |

---

*End of v0.20 specification.*
