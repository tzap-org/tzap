# tzap Archive Format Specification (v0.19)

| Field | Value |
|---|---|
| **Format version** | 1 |
| **Document version** | 0.19 / 2026-05-21.3 (draft after tar-stream and canonical-cursor review) |
| **Status** | Draft for review, no implementation yet |
| **Owner** | Frank Zhu |
| **Maintainers** | Frank Zhu |
| **Last updated** | 2026-05-21 |
| **Supersedes** | v0.1, v0.2, v0.3, v0.4, v0.5, v0.6, v0.7, v0.8, v0.9, v0.10, v0.11, v0.12, v0.13, v0.14, v0.15, v0.16, v0.17, v0.18 |
| **Superseded by** | None |
| **Conflict rule** | This document supersedes earlier tzap format drafts. If it conflicts with v0.1-v0.18 text, this v0.19 draft wins unless a later dated spec explicitly supersedes it. |
| **File extension** | `.tzap` (single-volume) / `.tzap.NNN` (multi-volume) |

## Changelog from v0.18

This revision resolves tar-stream boundary semantics and makes the
remaining v0.18 cursor and resource-cap rules fully executable.

1. **Draft visibility is bumped.** `volume_format_rev = 19` for this
   draft. (§8, §17.1, §23)
2. **Tar end-of-archive handling is defined.** The encrypted tzap tar
   stream excludes the POSIX two-zero-block end marker; readers append a
   synthetic marker only when presenting decoded bytes to a strict tar
   consumer. (§6.2, §15.2, §17.2, §17.3, §17.4, §19.1)
3. **Canonical cursor rules are explicit.** IndexRoot and IndexShard
   table offsets are now specified with step-by-step cursor rules,
   including zero-count behavior. (§15.9)
4. **Dictionary presence is validated in both directions.** Archives with
   `has_dictionary = 1` must carry a non-empty dictionary object, and
   `has_dictionary = 0` still requires all dictionary fields to be zero.
   (§15.2, §15.9)
5. **Sequential CRC failure wording is corrected.** A CRC-failed
   BlockRecord's kind/flags are untrusted; sequential readers treat it as
   an erasure and must not infer envelope-end flags from it. (§17.3)
6. **Reader-side caps are clarified.** `chunk_size`,
   `envelope_target_size`, and `block_size` caps are enforced before the
   validated values are treated as metadata; total extraction size and
   sidecar cap arithmetic are now unambiguous. (§9, §13.3, §29)
7. **IndexRoot FEC class maxima are less restrictive.** Actual object
   shard totals must fit ReedSolomonGF16; class maxima no longer need
   their sum to be ≤ 65,535. (§24)
8. **Smaller validation polish.** ManifestFooter IndexRoot size
   canonicality is stated explicitly, zero-shard directory hints are
   rejected, sidecar flag reserved bits are annotated in the struct, and
   half-open padding byte ranges are spelled out. (§6.1, §11, §12.3,
   §15.9)
...
a usable archive mode. Conformant v0.19 writers MUST set
`compression_algo = ZstdFramed`; conformant v0.19 readers MUST reject
...
- `pad_len = 0` is not valid in v0.19. The extra block in the exact-fit
...
stream), conforming v0.19 writers MUST use `stripe_width = 1`,
...
struct VolumeHeader {
    ...
    volume_format_rev:        u16,       // 19 for this draft
...
Readers MUST reject `compression_algo = None` in v0.19; payload and
...
| ~~`0x0004`~~ | ~~`[u8; 32]`~~ | **Forbidden in v0.19.** The tar-stream content hash is encrypted inside IndexRoot. Writers MUST NOT emit this extension; readers MUST reject it if present. |
...
struct BlockRecord {
    ...
    flags:         u8,               // bit 0: last data block of encrypted object
                                     // bits 1..7: reserved; MUST be zero in v0.19
    _reserved:     [u8; 2],
...
MUST reject a BlockRecord with any reserved flag bit set; in v0.19 this
...
Completed v0.19 writers MUST set `is_authoritative = 1` in every closed
...
struct BootstrapSidecarHeader {
    ...
    flags:                       u32,       // bit 0: ManifestFooter present
                                             ...
                                             // bits 3..31: reserved; MUST be zero in v0.19
...
No padding, extension bytes, or unclaimed gaps are permitted in a v0.19
...
non-critical by a future version; for v0.19, unknown flag bits are a hard
error.
...
overflow while computing the cap is a hard rejection before allocation.
...
For every encrypted object in v0.19, `encrypted_size` is the total
...
`FileEntry.flags` is reserved in v0.19. Writers MUST set it to zero, and
...
- when the same `frame_index` or `envelope_index` appears in more than
  ...
  version, this is equivalent to byte-identical row encoding in v0.19.
...
1. Read VolumeHeader at offset 0; verify CRC.
   ...
   only v0.19 conformance rejects `volume_format_rev != 19`.
...
be zero in v0.19 archives. Readers MUST reject blocks with reserved flag
bits set.
...
No part of the v0.19 write path needs to revisit a closed S3 part or to
...
writing final volumes. There is no conforming v0.19 mode that round-robins
...
### 20.1 Padding overhead (v0.19 unchanged from v0.15)
...
document uses `format_version = 1` and `volume_format_rev = 19`. Readers
...
`volume_format_rev = 19`; accepting earlier draft revisions requires an
...
implementation claims conformance to this v0.19 draft, incompatible
...
### 28.1 Test corpus additions for v0.19
...
- **Volume format revision**: create archives with `volume_format_rev`
  below, equal to, and above 19; verify v0.19-only readers accept only
  19 and reject newer revisions.
...
- **Compression registry enforcement**: mutate CryptoHeader to
  `compression_algo = None`; verify v0.19 readers reject it before
...
- **Sidecar cap arithmetic**: use large `block_size` and large
  ...
  wider arithmetic and reject on overflow.
...
1. Produces archives whose write sequence is strictly forward
   ...
   `format_version = 1` and `volume_format_rev = 19`.
...
35. Emits v0.19 bootstrap sidecars as packed sequences with no padding,
...
## Appendix A: All changes from v0.18 → v0.19

| Section | Change |
|---|---|
| §6.1 | Padding reader ranges clarified as explicit byte offsets / half-open ranges |
| §6.2 / §15.2 / §17.2 / §17.3 / §17.4 / §19.1 / §29 | tzap tar stream excludes POSIX end-of-archive marker; readers append a synthetic marker only for strict tar consumers or export |
| §9 / §13.3 / §29 | Reader caps for `chunk_size`, `envelope_target_size`, and `block_size` made enforceable before advisory use |
| §11 / §17.1 / §28.1 | ManifestFooter IndexRoot encrypted size canonicality stated explicitly |
| §12.3 | Bootstrap sidecar flags document reserved bits; CRC wording clarified as raw unkeyed corruption check |
| §13.3 | `max_shard_count` added; total extraction size input basis clarified; sidecar cap arithmetic requires checked u64-or-wider operations |
| §15.2 / §15.9 / §29 | `has_dictionary = 1` requires a non-empty dictionary object before payload decompression |
| §15.9 | IndexRoot and IndexShard canonical cursor rules written explicitly; `directory_hint_shard_count > 0` requires `shard_count > 0` |
| §15.9 / §29 | DirectoryHintEntry `shard_count = 0` is malformed |
| §17.3 / §28.1 | Sequential CRC failure wording corrected so kind/flags from a failed block are never trusted |
| §24 | IndexRoot FEC class maxima sum no longer has to fit 65,535; actual object totals still must |
| §28.1 / §29 | v0.19 tests and conformance checklist expanded |

---

*End of v0.19 specification.*
